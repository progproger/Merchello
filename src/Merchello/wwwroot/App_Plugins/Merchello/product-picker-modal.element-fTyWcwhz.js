import { LitElement as $, html as o, nothing as c, css as w, property as b, state as u, customElement as x } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalBaseElement as z } from "@umbraco-cms/backoffice/modal";
import { M as S } from "./merchello-api-658q9849.js";
import { c as O } from "./formatting-nNQcXJwZ.js";
...
let f = class extends A($) {
  ...
  _renderImage() {
    return this.variant.imageUrl && !this._imageFailed ? o`<img
...
f = y([
  x("merchello-product-picker-variant-row")
], f);
...
let _ = class extends A($) {
  ...
  _renderProductImage(e, i) {
    return e && !this._failedImages.has(e) ? o`<img
        src="${e}"
        alt="${i}"
        class="product-image"
...
_ = k([
  x("merchello-product-picker-list")
], _);
var L = Object.defineProperty, V = Object.getOwnPropertyDescriptor, h = (e, i, t, s) => {
...
let d = class extends z {
  ...
  async _loadVariantsForRoot(e) {
    const i = this._productRoots.findIndex((l) => l.id === e);
    if (i === -1) return;
    ...
    const a = await Promise.all(
      t.variants.map(async (l) => this._mapToPickerVariant(l, t))
    ), r = t.variants.reduce((l, g) => l + g.totalStock, 0), n = t.aggregateStockStatus;
    this._productRoots = this._productRoots.map(
      (l, g) => g === i ? { ...l, variants: a, variantsLoaded: !0, totalStock: r, stockStatus: n } : l
    );
  }
  async _mapToPickerVariant(e, i) {
    const t = e.totalStock, s = e.warehouseStock.some((p) => p.trackStock);
    let a = !0, r = null, n = !0, l = null, g = null, m = "InStock";
    if (!this._isPropertyEditorMode) {
      if (this._config?.shippingAddress && !i.isDigitalProduct) {
        const p = await this._getFulfillmentOptions(e.id);
        a = p.canAddToOrder, r = p.blockedReason, n = p.canAddToOrder, l = p.warehouseId, g = p.warehouseName, m = p.aggregateStockStatus;
      } else if (!i.isDigitalProduct && e.warehouseStock.length > 0) {
        const p = await this._getDefaultFulfillingWarehouse(e.id);
        a = p.canAddToOrder, r = p.blockedReason, l = p.warehouseId, g = p.warehouseName, m = p.aggregateStockStatus;
      }
    }
    let P = m;
    if (l) {
      const p = e.warehouseStock.find((I) => I.warehouseId === l);
      p && (P = p.stockStatus);
    }
    return {
      id: e.id,
  ...
  async _getFulfillmentOptions(e) {
    ...
    try {
      ...
      return s ? { canAddToOrder: !1, warehouseId: null, warehouseName: null, blockedReason: "Unable to check fulfillment", aggregateStockStatus: "OutOfStock" } : {
      ...
      return { canAddToOrder: !1, warehouseId: null, warehouseName: null, blockedReason: "Unable to check fulfillment", aggregateStockStatus: "OutOfStock" };
    }
  }
  /**
  ...
  async _getDefaultFulfillingWarehouse(e) {
    try {
      ...
      return t ? { canAddToOrder: !1, warehouseId: null, warehouseName: null, blockedReason: "Unable to check fulfillment", aggregateStockStatus: "OutOfStock" } : {
  ...
  async _fetchAddonPricePreview() {
    ...
    this._pendingAddonSelection && (s ? (this._addonPricePreview = null, this._addonPreviewError = "Unable to calculate price. Please try again.") : t && (this._addonPricePreview = {
  ...
  async _transitionToShippingSelection(e, i) {
    ...
    const { data: n, error: l } = await S.getShippingOptionsForWarehouse(
      t,
      r.countryCode,
      r.stateCode
    );
    ...
    const g = n.availableOptions.map((m) => ({
      id: m.id,
      name: m.name,
  ...
  _renderAddonSelectionView() {
    const e = this._pendingAddonSelection;
    if (!e) return c;
    const i = e.variant, t = i.optionValuesDisplay ? `${i.rootName} - ${i.optionValuesDisplay}` : i.rootName, s = this._addonPricePreview, a = this._addonPreviewError !== null, r = s?.basePrice ?? i.price, n = s?.addonsTotal ?? 0, l = s?.totalPrice ?? i.price, g = !a && !this._isLoadingAddonPreview;
    return o`
      <umb-body-layout headline="Select Add-ons (Optional)">
        <div id="main">
          ...
          <div class="addon-product-summary">
            ...
            <div class="product-pricing ${this._isLoadingAddonPreview ? "loading" : ""} ${a ? "error" : ""}">
              ...
              ${!a && n !== 0 ? o`
                    ...
                    <span class="total-price">= ${v(l, this._currencySymbol)}</span>
                  ` : c}
          ...
          <div class="addon-options">
            ${e.addonOptions.map((m) => this._renderAddonOption(m))}
          </div>
        </div>
...
d = h([
  x("merchello-product-picker-modal")
], d);
const q = d;
export {
...
//# sourceMappingURL=product-picker-modal.element-fTyWcwhz.js.map
