import { html as a, nothing as h, css as k, state as d, customElement as C } from "@umbraco-cms/backoffice/external/lit";
...
import { a as _ } from "./formatting-MfE1tvkN.js";
import { m as O } from "./modal-layout.styles-C2OaUji5.js";
const R = new v("Merchello.ShippingCost.Modal", {
  modal: {
    type: "dialog",
    size: "medium"
  }
...
}), I = new v("Merchello.ShippingPostcodeRule.Modal", {
  modal: {
    type: "dialog",
    size: "medium"
  }
});
var W = Object.defineProperty, A = Object.getOwnPropertyDescriptor, x = (e) => {
  throw TypeError(e);
}, n = (e, t, i, s) => {
  for (var r = s > 1 ? void 0 : s ? A(t, i) : t, g = e.length - 1, m; g >= 0; g--)
    (m = e[g]) && (r = (s ? m(t, i, r) : m(r)) || r);
  return s && r && W(t, i, r), r;
}, w = (e, t, i) => t.has(e) || x("Cannot " + i), o = (e, t, i) => (w(e, t, "read from private field"), t.get(e)), f = (e, t, i) => t.has(e) ? x("Cannot add the same private member more than once") : t instanceof WeakSet ? t.add(e) : t.set(e, i), D = (e, t, i, s) => (w(e, t, "write to private field"), t.set(e, i), i), c, u;
let l = class extends S {
  constructor() {
    super(), this._isLoading = !1, this._isSaving = !1, this._errorMessage = null, this._detail = null, this._warehouses = [], this._excludedRegions = [], this._name = "", this._warehouseId = "", this._fixedCost = 0, this._daysFrom = 3, this._daysTo = 5, this._isNextDay = !1, this._nextDayCutOffTime = "", this._allowsDeliveryDateSelection = !1, this._minDeliveryDays = null, this._maxDeliveryDays = null, this._allowedDaysOfWeek = "", this._isDeliveryDateGuaranteed = !1, this._isEnabled = !0, this._activeTab = "overview", f(this, c), f(this, u), this._daysOfWeek = [
  ...
  get _isFlatRateOption() {
    return (this._detail?.providerKey ?? this.data?.option?.providerKey ?? "flat-rate").toLowerCase() === "flat-rate";
  }
  connectedCallback() {
  ...
  async _save() {
    ...
    this._isSaving = !0;
    const e = this._isFlatRateOption, t = {
      name: this._name,
      warehouseId: this._warehouseId,
      fixedCost: e ? this._fixedCost ?? 0 : this._fixedCost ?? void 0,
    ...
    try {
      const i = this.data?.option?.id || this.data?.optionId || this._detail?.id, s = !i, r = i ? await p.updateShippingOption(i, t) : await p.createShippingOption(t);
      if (r.error) {
        o(this, u)?.peek("danger", {
          data: { headline: "Error", message: r.error.message }
        }), this._isSaving = !1;
        return;
      }
      if (s && r.data) {
        this._detail = r.data, this._warehouseId = r.data.warehouseId, this._fixedCost = this._isFlatRateOption ? r.data.fixedCost ?? 0 : r.data.fixedCost ?? null, this._excludedRegions = r.data.excludedRegions ?? this._excludedRegions, this._activeTab = "pricing", o(this, u)?.peek("positive", {
          data: {
  ...
  async _openCostModal(e) {
    if (!o(this, c) || !this._detail) return;
    (await o(this, c).open(this, R, {
      data: { cost: e, optionId: this._detail.id, warehouseId: this._detail.warehouseId }
    }).onSubmit().catch(() => {
    }))?.isSaved && await this._loadDetail();
  }
  async _deleteCost(e) {
    const t = e.regionDisplay ?? e.countryCode, i = o(this, c)?.open(this, b, {
      data: {
        headline: "Delete Shipping Cost",
        content: `Delete the shipping cost for ${t}.`,
        confirmLabel: "Delete",
        color: "danger"
      }
    });
    ...
    const { error: s } = await p.deleteShippingCost(e.id);
    if (s) {
      o(this, u)?.peek("danger", {
  ...
  async _deleteWeightTier(e) {
    ...
    const { error: s } = await p.deleteShippingWeightTier(e.id);
    if (s) {
      o(this, u)?.peek("danger", {
        data: { headline: "Error", message: s.message }
      });
  ...
  async _openExclusionModal(e) {
    if (!o(this, c)) return;
    const i = await o(this, c).open(this, M, {
      data: {
        exclusion: e,
    ...
    if (!i?.isSaved || !i.exclusion)
      return;
    const s = i.exclusion.countryCode.toUpperCase(), r = i.exclusion.regionCode?.toUpperCase();
    if (this._excludedRegions.find((y) => y !== e && y.countryCode.toUpperCase() === s && (y.regionCode ?? "").toUpperCase() === (r ?? ""))) {
      o(this, u)?.peek("warning", {
    ...
    const m = {
      ...
      countryCode: s,
      regionCode: r,
      regionDisplay: this._buildExclusionRegionDisplay(s, r)
    };
  ...
  async _deleteExclusion(e) {
    const t = e.regionDisplay ?? this._buildExclusionRegionDisplay(e.countryCode, e.regionCode), i = o(this, c)?.open(this, b, {
      data: {
    ...
    this._excludedRegions = this._excludedRegions.filter((s) => s.id !== e.id);
  }
  async _openPostcodeRuleModal(e) {
    if (!o(this, c) || !this._detail) return;
    (await o(this, c).open(this, I, {
      data: { rule: e, optionId: this._detail.id }
    }).onSubmit().catch(() => {
    }))?.isSaved && await this._loadDetail();
  }
  async _deletePostcodeRule(e) {
    const t = e.pattern || e.id, i = o(this, c)?.open(this, b, {
      data: {
        headline: "Delete Postcode Rule",
        content: `Delete postcode rule "${t}".`,
        confirmLabel: "Delete",
        color: "danger"
      }
    });
    ...
    const { error: s } = await p.deleteShippingPostcodeRule(e.id);
    if (s) {
      o(this, u)?.peek("danger", {
        data: { headline: "Error", message: s.message }
      });
  ...
  _toggleDay(e) {
    ...
    if (t.includes(e))
      ...
      const i = this._daysOfWeek.map((r) => r.value), s = [...t, e].sort(
        (r, g) => i.indexOf(r) - i.indexOf(g)
      );
      this._allowedDaysOfWeek = s.join(",");
    }
  }
        ...
        <div class="form-grid">
          ...
          <uui-form-layout-item>
            ...
            <uui-input
              ...
              @input=${(e) => {
      const t = e.target.value, i = Number.parseFloat(t), s = Number.isFinite(i) ? i : null;
  ...
  _renderDeliveryTab() {
    return a`
      ...
      <uui-box headline="Delivery Date Selection" class="optional-section">
        ...
        ${this._allowsDeliveryDateSelection ? a`
  ...
  _renderCostsTable() {
    return this._detail ? a`
      <uui-box headline="Shipping Rates">
        ...
        ${this._detail.costs.length === 0 ? a`<p class="no-items">No destination rates configured. Fixed Cost will be used as fallback for all destinations.</p>` : a`
  ...
  _renderWeightTiersTable() {
    return this._detail ? a`
      <uui-box headline="Weight Surcharges">
        ...
        ${this._detail.weightTiers.length === 0 ? a`<p class="no-items">No weight surcharges configured.</p>` : a`
              <table class="data-table">
                <thead>
                  <tr>
                    <th>Destination</th>
  ...
  _renderExcludedRegionsTable() {
    return a`
      <uui-box headline="Destination Exclusions">
        ...
        ${this._excludedRegions.length === 0 ? a`<p class="no-items">No destinations excluded.</p>` : a`
              <table class="data-table">
                <thead>
                  <tr>
                    <th>Destination</th>
      ...
      const i = `${e.countryCode}:${e.regionCode ?? ""}`, s = `${t.countryCode}:${t.regionCode ?? ""}`;
  ...
  _renderPostcodeRulesTable() {
    return this._detail ? a`
      <uui-box headline="Postcode Rules">
        ...
        ${(this._detail.postcodeRules?.length ?? 0) === 0 ? a`<p class="no-items">No postcode rules configured.</p>` : a`
              <table class="data-table">
                <thead>
                  <tr>
                    <th>Country</th>
      ...
      (e) => a`
                      <tr>
                        ...
                        <td>
                          ${e.action === "Block" ? a`<span class="rule-action rule-action-block">Block</span>` : a`<span class="rule-action rule-action-surcharge">+${_(e.surcharge ?? 0)}</span>`}
                        </td>
                        <td class="actions-col">
    ...
    return this._isLoading ? a`
      ...
      <umb-body-layout headline="${e ? "Edit" : "Add"} Shipping Option">
        ...
        <uui-button
          ...
          @click=${this._save}>
          ${this._isSaving ? a`<uui-loader-circle></uui-loader-circle>` : h}
          ${e ? "Save" : "Create"}
        </uui-button>
      </umb-body-layout>
    `;
...
n([
  d()
], l.prototype, "_excludedRegions", 2);
n([
  d()
], l.prototype, "_name", 2);
n([
  d()
], l.prototype, "_warehouseId", 2);
n([
  d()
], l.prototype, "_fixedCost", 2);
n([
  d()
], l.prototype, "_daysFrom", 2);
n([
  d()
], l.prototype, "_daysTo", 2);
n([
  d()
], l.prototype, "_isNextDay", 2);
n([
  d()
], l.prototype, "_nextDayCutOffTime", 2);
n([
  d()
], l.prototype, "_allowsDeliveryDateSelection", 2);
n([
  d()
], l.prototype, "_minDeliveryDays", 2);
n([
  d()
], l.prototype, "_maxDeliveryDays", 2);
n([
  d()
], l.prototype, "_allowedDaysOfWeek", 2);
n([
  d()
], l.prototype, "_isDeliveryDateGuaranteed", 2);
n([
  d()
], l.prototype, "_isEnabled", 2);
n([
  d()
...
const U = l;
export {
  l as MerchelloShippingOptionDetailModalElement,
  U as default
};
//# sourceMappingURL=shipping-option-detail-modal.element-CUqCe8fi.js.map
