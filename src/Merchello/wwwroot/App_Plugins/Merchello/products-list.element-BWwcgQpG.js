import { LitElement as P, nothing as w, html as r, css as k, property as p, state as l, customElement as S } from "@umbraco-cms/backoffice/external/lit";
import { UmbElementMixin as x } from "@umbraco-cms/backoffice/element-api";
import { UmbModalToken as I, UMB_MODAL_MANAGER_CONTEXT as A } from "@umbraco-cms/backoffice/modal";
import { M as v } from "./merchello-api-658q9849.js";
import { g as E } from "./store-settings-B0mEEUwl.js";
import { b as M, c as z } from "./navigation-Y0bwD8V1.js";
import "./pagination.element-sDi4Myhy.js";
import "./merchello-empty-state.element-mt97UoA5.js";
import { a as f } from "./formatting-nNQcXJwZ.js";
...
let _ = class extends x(P) {
  ...
  _renderPopover() {
    return !this._isOpen || this.warnings.length === 0 ? w : r`
      <div class="popover">
...
let d = class extends x(P) {
  ...
  _renderCell(e, t) {
    switch (t) {
      case "select":
        ...
        `;
      case "rootName":
        return r`<uui-table-cell class="product-name"><a href=${M(e.productRootId)}>${e.rootName}</a></uui-table-cell>`;
      case "sku":
  ...
  _formatPriceRange(e) {
    return e.minPrice != null && e.maxPrice != null && e.minPrice !== e.maxPrice ? `${f(e.minPrice)} - ${f(e.maxPrice)}` : f(e.price);
  }
  _renderRow(e) {
    const t = this._getEffectiveColumns();
    return r`
      <uui-table-row class=${this.clickable ? "clickable" : ""} @click=${() => this._handleRowClick(e)}>
        ${t.map((i) => this._renderCell(e, i))}
      </uui-table-row>
    `;
  }
  render() {
...
let o = class extends x(P) {
  ...
  connectedCallback() {
    super.connectedCallback(), y(this, h, !0), this._initializeAndLoad();
  }
  async _initializeAndLoad() {
    const e = await E();
    m(this, h) && (this._pageSize = e.defaultPaginationPageSize, this._loadFilterOptions(), this._loadProducts());
  }
  disconnectedCallback() {
    super.disconnectedCallback(), y(this, h, !1), this._searchDebounceTimer && clearTimeout(this._searchDebounceTimer);
  }
  async _loadFilterOptions() {
    const [e, t] = await Promise.all([
      v.getProductTypes(),
      v.getProductCollections()
    ]);
    m(this, h) && (e.data && (this._productTypes = e.data), t.data && (this._collections = t.data));
  }
  async _loadProducts() {
    ...
    const { data: t, error: i } = await v.getProducts(e);
    if (m(this, h)) {
      if (i) {
  ...
  async _handleAddProduct() {
    ...
    t?.isCreated && t.productId && z(t.productId);
  }
...
//# sourceMappingURL=products-list.element-BWwcgQpG.js.map
