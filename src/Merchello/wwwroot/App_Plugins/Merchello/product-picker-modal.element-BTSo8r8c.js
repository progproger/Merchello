import { LitElement as P, html as i, nothing as d, css as $, property as f, customElement as w, state as p } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalBaseElement as z } from "@umbraco-cms/backoffice/modal";
import { M as y } from "./merchello-api-BW8jq3Oc.js";
import { c as R } from "./formatting-D9W2VVNu.js";
...
var N = Object.defineProperty, D = Object.getOwnPropertyDescriptor, x = (e, t, a, s) => {
  for (var r = s > 1 ? void 0 : s ? D(t, a) : t, o = e.length - 1, n; o >= 0; o--)
    (n = e[o]) && (r = (s ? n(t, a, r) : n(r)) || r);
  return s && r && N(t, a, r), r;
};
let m = class extends C(P) {
  ...
  _renderImage() {
    return this.variant.imageUrl ? i`<img src="${this.variant.imageUrl}" alt="${this.variant.name ?? ""}" class="variant-image" />` : i`
  ...
  _renderName() {
    ...
    return i`<span class="variant-name">${e}</span>`;
  }
  _renderSku() {
    return this.variant.sku ? i`<span class="variant-sku">${this.variant.sku}</span>` : d;
  }
  _renderPrice() {
    return i`<span class="variant-price">${b(this.variant.price, this.currencySymbol)}</span>`;
  }
  _renderStockStatus() {
    return this.variant.trackStock ? this.variant.availableStock <= 0 ? i`<span class="status blocked">Out of stock</span>` : this.variant.availableStock <= 5 ? i`<span class="status warning">Low: ${this.variant.availableStock}</span>` : i`<span class="status available">${this.variant.availableStock} in stock</span>` : i`<span class="status available">Available</span>`;
  }
  _renderRegionStatus() {
    return this.variant.canShipToRegion ? d : i`<span class="status blocked">${this.variant.regionMessage ?? "Cannot ship"}</span>`;
  }
  _renderBlockedReason() {
    return !this.variant.canSelect && this.variant.blockedReason ? i`
...
var M = Object.defineProperty, O = Object.getOwnPropertyDescriptor, S = (e, t, a, s) => {
  for (var r = s > 1 ? void 0 : s ? O(t, a) : t, o = e.length - 1, n; o >= 0; o--)
    (n = e[o]) && (r = (s ? n(t, a, r) : n(r)) || r);
...
let v = class extends C(P) {
  ...
  _renderProductImage(e, t) {
    return e ? i`<img src="${e}" alt="${t}" class="product-image" />` : i`
  ...
  _renderStockBadge(e) {
    return e.isDigitalProduct ? i`<span class="badge digital">Digital</span>` : e.totalStock <= 0 ? i`<span class="badge out-of-stock">Out of stock</span>` : e.totalStock <= 5 ? i`<span class="badge low-stock">Low: ${e.totalStock}</span>` : i`<span class="badge in-stock">${e.totalStock} in stock</span>`;
  }
  _renderExpandIcon(e) {
    return i`
  ...
  _renderProductRoot(e) {
    ...
    return i`
      <div class="product-root ${e.isExpanded ? "expanded" : ""}">
        ...
        >
          ${t ? i`<div class="expand-spacer"></div>` : this._renderExpandIcon(e.isExpanded)}
          ...
          <div class="product-info">
            ...
            <div class="product-meta">
              ...
              ${t ? d : i`<span class="variant-count">${e.variantCount} variants</span>`}
              ${this._renderStockBadge(e)}
            </div>
          </div>
        </button>

        ${e.isExpanded && e.variantsLoaded ? i`
  ...
  render() {
    return this.productRoots.length === 0 ? i`<div class="empty">No products to display</div>` : i`
...
var V = Object.defineProperty, j = Object.getOwnPropertyDescriptor, u = (e, t, a, s) => {
  for (var r = s > 1 ? void 0 : s ? j(t, a) : t, o = e.length - 1, n; o >= 0; o--)
    (n = e[o]) && (r = (s ? n(t, a, r) : n(r)) || r);
...
let l = class extends z {
  ...
  async _loadProducts() {
    ...
    this._searchTerm.trim() && (e.search = this._searchTerm.trim()), this._config?.productTypeId && (e.productTypeId = this._config.productTypeId), this._config?.categoryId && (e.categoryId = this._config.categoryId);
    ...
    if (t) {
      ...
      this._productRoots = t.items.filter((r) => !s.includes(r.productRootId)).map((r) => this._mapToPickerRoot(r)), this._totalPages = t.totalPages;
    }
    this._isLoading = !1;
  }
  _mapToPickerRoot(e) {
  ...
  async _loadVariantsForRoot(e) {
    ...
    this._productRoots = this._productRoots.map(
      (o, n) => n === t ? { ...o, variants: r, variantsLoaded: !0 } : o
    );
  }
  async _mapToPickerVariant(e, t) {
    ...
    let g = !0, k = null;
    return s && a <= 0 && (g = !1, k = "Out of stock"), g && !r && (g = !1, k = "Cannot ship to region"), g && !e.availableForPurchase && (g = !1, k = "Not available for purchase"), {
      id: e.id,
      productRootId: t.id,
      name: e.name,
      rootName: t.rootName,
  ...
  async _canWarehouseServeRegion(e, t, a) {
    ...
    if (!a)
      return !0;
    const r = `${e}:${t}`;
    if (!this._regionCache.regions.has(r)) {
      const { data: n } = await y.getAvailableRegionsForWarehouse(e, t);
      n && n.length > 0 ? this._regionCache.regions.set(r, new Set(n.map((h) => h.regionCode))) : this._regionCache.regions.set(r, null);
    }
    const o = this._regionCache.regions.get(r);
  ...
  _renderContent() {
    return this._isLoading ? i`<div class="loading"><uui-loader></uui-loader></div>` : this._errorMessage ? i`<div class="error">${this._errorMessage}</div>` : this._productRoots.length === 0 ? i`
        <div class="empty">
  ...
  _renderSelectionSummary() {
    ...
    return e === 0 ? i`<span class="selection-count">No products selected</span>` : i`<span class="selection-count">${e} product${e === 1 ? "" : "s"} selected</span>`;
  }
  render() {
    return i`
...
//# sourceMappingURL=product-picker-modal.element-BTSo8r8c.js.map
