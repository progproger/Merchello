...
async function r(e) {
  try {
    ...
    let u;
    return a.includes("application/json") ? u = await o.json() : u = await o.text(), { data: u };
  } catch (t) {
...
const w = {
  ...
  downloadCustomerStatement: async (e, t, s) => {
    try {
      const o = await $(), a = d.baseUrl || "", u = new URLSearchParams();
      t && u.append("periodStart", t), s && u.append("periodEnd", s);
      const g = u.toString(), y = `${a}${m}/customers/${e}/statement${g ? `?${g}` : ""}`, l = await fetch(y, {
        method: "GET",
        credentials: d.credentials,
      ...
      const f = await l.blob(), v = l.headers.get("content-disposition");
      let S = "statement.pdf";
      if (v) {
        const h = v.match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/);
        h && h[1] && (S = h[1].replace(/['"]/g, ""));
      }
      return { blob: f, filename: S };
    } catch (o) {
...
//# sourceMappingURL=merchello-api-BAKL0aIE.js.map
