import { LitElement as O, html as s, nothing as h, css as S, state as d, customElement as x } from "@umbraco-cms/backoffice/external/lit";
import { UmbElementMixin as k } from "@umbraco-cms/backoffice/element-api";
import { UMB_WORKSPACE_CONTEXT as P } from "@umbraco-cms/backoffice/workspace";
import { UmbModalToken as C, UMB_MODAL_MANAGER_CONTEXT as R, UMB_CONFIRM_MODAL as y } from "@umbraco-cms/backoffice/modal";
import { UMB_NOTIFICATION_CONTEXT as T } from "@umbraco-cms/backoffice/notification";
import { M as m } from "./merchello-api-LENiBVrz.js";
import { M as A } from "./supplier-modal.token-CWeQ_zlc.js";
import { m as M, z as I, A as w } from "./navigation-COkStlQk.js";
import { b as E } from "./badge.styles-DUcdl6GY.js";
import { a as z } from "./formatting-CeWY__1B.js";
const L = new C("Merchello.ServiceRegion.Modal", {
  modal: {
    type: "sidebar",
    size: "medium"
  }
}), F = new C("Merchello.ShippingOption.Detail.Modal", {
  modal: {
    type: "sidebar",
    size: "large"
  }
});
var N = Object.defineProperty, W = Object.getOwnPropertyDescriptor, $ = (e) => {
  throw TypeError(e);
}, u = (e, i, a, c) => {
  for (var n = c > 1 ? void 0 : c ? W(i, a) : i, f = e.length - 1, _; f >= 0; f--)
    (_ = e[f]) && (n = (c ? _(i, a, n) : _(n)) || n);
  return c && n && N(i, a, n), n;
}, D = (e, i, a) => i.has(e) || $("Cannot " + a), t = (e, i, a) => (D(e, i, "read from private field"), i.get(e)), v = (e, i, a) => i.has(e) ? $("Cannot add the same private member more than once") : i instanceof WeakSet ? i.add(e) : i.set(e, a), b = (e, i, a, c) => (D(e, i, "write to private field"), i.set(e, a), a), l, g, p, r;
function q() {
  return {
    name: null,
    email: null,
...
let o = class extends k(O) {
  constructor() {
    super(), this._warehouse = null, this._isLoading = !0, this._isSaving = !1, this._errorMessage = null, this._routes = [], this._activePath = "", this._formData = {}, this._suppliers = [], this._countries = [], this._shippingOptions = [], this._fulfilmentProviderOptions = [], this._isDeletingRegion = null, this._isDeletingOption = null, v(this, l), v(this, g), v(this, p), v(this, r, !1), this.consumeContext(P, (e) => {
      b(this, l, e), t(this, l) && this.observe(t(this, l).warehouse, (i) => {
  ...
  connectedCallback() {
    super.connectedCallback(), b(this, r, !0), this._createRoutes(), this._loadSuppliers(), this._loadCountries(), this._loadFulfilmentProviders();
  }
  disconnectedCallback() {
    super.disconnectedCallback(), b(this, r, !1);
  }
  // Tab routing
  _createRoutes() {
  ...
  async _loadFulfilmentProviders() {
    ...
    t(this, r) && e && (this._fulfilmentProviderOptions = e);
  }
  ...
  _handleAddressChange(e, i) {
    const a = this._formData.address ?? q();
    this._formData = {
      ...this._formData,
      address: {
        ...a,
  ...
  _handleFulfilmentProviderChange(e) {
    const i = e.target;
    this._formData = { ...this._formData, fulfilmentProviderConfigurationId: i.value || void 0 };
  }
  ...
  async _openCreateSupplierModal() {
    if (!t(this, g)) return;
    const i = await t(this, g).open(this, A, {
      data: {}
    }).onSubmit().catch(() => {
    });
  ...
  async _handleSave() {
    ...
    try {
      if (e) {
        const { data: i, error: a } = await m.createWarehouse({
        ...
        if (!t(this, r)) return;
        if (a) {
          this._errorMessage = a.message, t(this, p)?.peek("danger", {
            data: { headline: "Failed to create", message: a.message || "Could not create warehouse" }
          });
          return;
        }
        i && (t(this, l)?.updateWarehouse(i), history.replaceState({}, "", M(i.id)), t(this, p)?.peek("positive", {
          data: { headline: "Warehouse created", message: "The warehouse has been created successfully" }
        }));
      } else {
        const { data: i, error: a } = await m.updateWarehouse(this._warehouse.id, {
          name: this._formData.name,
          code: this._formData.code,
          supplierId: this._formData.supplierId,
        ...
        i && (t(this, l)?.updateWarehouse(i), t(this, p)?.peek("positive", {
  ...
  async _handleDelete() {
    ...
    if (t(this, r)) {
      if (i) {
        this._errorMessage = `Failed to delete warehouse: ${i.message}`, t(this, p)?.peek("danger", {
  ...
  async _openServiceRegionModal(e) {
    if (!t(this, g) || !this._warehouse?.id) return;
    const a = await t(this, g).open(this, L, {
      data: {
  ...
  async _handleDeleteRegion(e, i) {
    ...
    const a = i.regionDisplay || `${i.stateOrProvinceCode || ""} ${i.countryCode}`.trim(), c = t(this, g)?.open(this, y, {
      data: {
    ...
    if (t(this, r)) {
      if (this._isDeletingRegion = null, n) {
        this._errorMessage = `Failed to delete region: ${n.message}`, t(this, p)?.peek("danger", {
  ...
  async _openShippingOptionModal(e) {
    if (!t(this, g) || !this._warehouse?.id) return;
    const a = await t(this, g).open(this, F, {
      data: {
        optionId: e?.id,
        warehouseId: this._warehouse.id
      }
  ...
  async _handleDeleteOption(e, i) {
    ...
    const { error: c } = await m.deleteShippingOption(i.id);
    if (t(this, r)) {
      if (this._isDeletingOption = null, c) {
        this._errorMessage = `Failed to delete option: ${c.message}`, t(this, p)?.peek("danger", {
          data: { headline: "Failed to delete", message: c.message || "Could not delete shipping option" }
        });
        return;
      }
      t(this, p)?.peek("positive", {
  ...
  _renderValidationSummary() {
    const e = [];
    return this._isFieldEmpty(this._formData.name) && e.push("Warehouse name is required"), this._isFieldEmpty(this._formData.address?.addressOne) && e.push("Address line 1 is required"), this._isFieldEmpty(this._formData.address?.townCity) && e.push("Town/City is required"), this._isFieldEmpty(this._formData.address?.postalCode) && e.push("Postal code is required"), this._isFieldEmpty(this._formData.address?.countryCode) && e.push("Country is required"), e.length === 0 ? h : s`
  ...
  _renderOptionsTab() {
    const e = t(this, l)?.isNew ?? !0, i = this._shippingOptions.some((a) => a.usesLiveRates);
    return s`
      <div class="tab-content">
        ${e ? s`
              <div class="info-banner warning">
  ...
  _renderOptionRow(e) {
    const i = this._isDeletingOption === e.id, a = e.usesLiveRates, c = e.isNextDay ? "Next Day" : e.daysFrom === e.daysTo ? `${e.daysFrom} days` : `${e.daysFrom}-${e.daysTo} days`;
    let n;
    return a ? n = "Live Rates" : e.fixedCost != null ? n = z(e.fixedCost) : e.costCount > 0 ? n = `${e.costCount} location(s)` : n = "—", s`
      <uui-table-row class="clickable" @click=${() => this._openShippingOptionModal(e)}>
        <uui-table-cell>
          ...
          ${e.serviceType ? s`<span class="service-type">${e.serviceType}</span>` : h}
        </uui-table-cell>
        <uui-table-cell>
          ...
          <uui-tab-group slot="header">
            ...
            <uui-tab
              ...
              <span class="tab-label">
                Service Regions${(this._warehouse?.serviceRegions.length ?? 0) === 0 ? s`<uui-icon name="icon-alert" class="tab-warning"></uui-icon>` : h}
            ...
            <uui-tab
              ...
              <span class="tab-label">
                Shipping Options${(this._warehouse?.shippingOptionCount ?? 0) === 0 ? s`<uui-icon name="icon-alert" class="tab-warning"></uui-icon>` : h}
...
//# sourceMappingURL=warehouse-detail.element-BKU_ai21.js.map
