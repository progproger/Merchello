import { LitElement as V, nothing as l, html as o, css as E, property as x, state as c, customElement as O } from "@umbraco-cms/backoffice/external/lit";
import { UmbElementMixin as F } from "@umbraco-cms/backoffice/element-api";
import { UMB_WORKSPACE_CONTEXT as N } from "@umbraco-cms/backoffice/workspace";
import { UmbModalToken as z, UMB_MODAL_MANAGER_CONTEXT as R, UMB_CONFIRM_MODAL as P } from "@umbraco-cms/backoffice/modal";
...
import { s as j, t as W } from "./navigation-BGhEgega.js";
import { a as C } from "./formatting-DGOWmvpU.js";
import { UmbChangeEvent as K } from "@umbraco-cms/backoffice/event";
import "./product-shipping-exclusions.element-tTZaGdN6.js";
...
import "./property-editor-ui-collection-picker.element-URRerHHd.js";
var q = Object.defineProperty, Y = Object.getOwnPropertyDescriptor, T = (t, e, i, a) => {
  for (var r = a > 1 ? void 0 : a ? Y(e, i) : e, n = t.length - 1, u; n >= 0; n--)
...
), X = new z("Merchello.VariantBatchUpdate.Modal", {
  modal: {
    type: "dialog",
    size: "large"
  }
});
var J = Object.defineProperty, Q = Object.getOwnPropertyDescriptor, $ = (t, e, i, a) => {
  for (var r = a > 1 ? void 0 : a ? Q(e, i) : e, n = t.length - 1, u; n >= 0; n--)
    (u = t[n]) && (r = (a ? u(e, i, r) : u(r)) || r);
  return a && r && J(e, i, r), r;
};
let y = class extends F(V) {
  constructor() {
...
}, U = (t, e, i) => e.has(t) || M("Cannot " + i), s = (t, e, i) => (U(t, e, "read from private field"), e.get(t)), D = (t, e, i) => e.has(t) ? M("Cannot add the same private member more than once") : e instanceof WeakSet ? e.add(t) : e.set(t, i), k = (t, e, i, a) => (U(t, e, "write to private field"), e.set(t, i), i), I, h, b, g, _;
let d = class extends F(V) {
  constructor() {
    super(), this._product = null, this._isLoading = !0, this._isSaving = !1, this._errorMessage = null, this._validationAttempted = !1, this._fieldErrors = {}, this._routes = [], this._activePath = "", this._formData = {}, this._variantFormData = {}, this._variantFieldErrors = {}, this._selectedVariantIds = [], this._taxGroups = [], this._productTypes = [], this._warehouses = [], this._productViews = [], this._optionSettings = null, this._elementTypes = [], this._filterGroups = [], this._assignedFilterIds = [], this._originalAssignedFilterIds = [], this._shippingOptions = [], this._elementType = null, this._elementPropertyValues = {}, this._descriptionEditorConfig = void 0, this._collectionPickerConfig = new S([
      { alias: "maxItems", value: 0 }
    ]), this._descriptionBlocks = null, D(this, I, new H(this)), D(this, h), D(this, b), D(this, g), D(this, _, !1), this.consumeContext(N, (t) => {
      k(this, h, t), s(this, h) && (this.observe(s(this, h).product, (e) => {
        this._product = e ?? null, e && (this._formData = { ...e }, this._descriptionBlocks = null, this._shippingOptions = e.availableShippingOptions ?? [], this._selectedVariantIds = [], e.variants.length === 1 && (this._variantFormData = { ...e.variants[0] }, this._loadAssignedFilters()), e.elementProperties && (this._elementPropertyValues = { ...e.elementProperties })), this._isLoading = !e;
  ...
  connectedCallback() {
    super.connectedCallback(), k(this, _, !0), this._loadReferenceData(), this._createRoutes();
  }
  ...
  async _loadDataTypeConfig(t) {
    try {
      const { error: e } = await s(this, I).requestByUnique(t);
      if (e) {
        this._setFallbackEditorConfig();
        return;
      }
      this.observe(
        await s(this, I).byUnique(t),
        (i) => {
          if (s(this, _)) {
            if (!i) {
              this._setFallbackEditorConfig();
            ...
            this._descriptionEditorConfig = new S(i.values);
          }
        },
  ...
  _setFallbackEditorConfig() {
    this._descriptionEditorConfig = new S([
      {
        alias: "toolbar",
        value: [
          [
  ...
  _handleCollectionIdsChange(t) {
    const a = (t.target.value ?? "").split(",").map((r) => r.trim()).filter(Boolean);
    this._formData = { ...this._formData, collectionIds: a };
  }
  _getViewOptions() {
  ...
  _getElementTypeSelection() {
    ...
    if (!t) return [];
    const e = this._elementTypes.find((i) => i.alias.toLowerCase() === t.toLowerCase());
    return e ? [e.key] : [];
  }
  async _handleElementTypeChange(t) {
    const a = (t.target.selection ?? [])[0];
    let r = this._elementTypes.find((m) => m.key === a);
    if (a && !r) {
      const { data: m } = await f.getElementTypes();
      m && (this._elementTypes = m, r = m.find((v) => v.key === a));
    }
    const n = r?.alias ?? null, u = this._formData.elementTypeAlias ?? null;
    n !== u && (this._formData = { ...this._formData, elementTypeAlias: n }, this._elementPropertyValues = {}, s(this, h)?.setElementPropertyValues({}), await s(this, h)?.loadElementType(n));
  }
  async _handleSave() {
  ...
  async _createProduct() {
    ...
    e && (s(this, h)?.updateProduct(e), s(this, g)?.peek("positive", { data: { headline: "Product created", message: `"${e.rootName}" has been created successfully` } }), this._validationAttempted = !1, this._fieldErrors = {});
  }
  async _updateProduct() {
    ...
    this._isSingleVariant() && await this._saveFilterAssignments(), await this._saveShippingExclusions(), e && (await s(this, h)?.reload(), s(this, g)?.peek("positive", { data: { headline: "Product saved", message: "Changes have been saved successfully" } }));
  }
  ...
  _renderGoogleSearchPreview() {
    const t = this._formData.pageTitle || this._formData.rootName || "Product Title", e = this._formData.metaDescription || "No meta description set. Add a description to improve search visibility.", i = this._formData.canonicalUrl || "https://yourstore.com/products/product-name", a = this._formatUrlAsBreadcrumb(i), r = 60, n = 160, u = t.length > r, m = e.length > n, v = u ? t.substring(0, r - 3) + "..." : t, G = m ? e.substring(0, n - 3) + "..." : e;
    return o`
  ...
  async _openBatchUpdateModal() {
    ...
    }), this._selectedVariantIds = [], await s(this, h)?.reload());
  }
  ...
  async _handleSetDefaultVariant(t) {
    ...
    try {
      ...
      r ? (s(this, g)?.peek("danger", { data: { headline: "Failed to set default variant", message: r.message } }), await s(this, h)?.reload()) : (s(this, g)?.peek("positive", { data: { headline: "Default variant updated", message: "" } }), await s(this, h)?.reload());
    } catch {
      s(this, g)?.peek("danger", { data: { headline: "Error", message: "An unexpected error occurred" } }), await s(this, h)?.reload();
    }
  }
  /**
  ...
  _renderOptionsTab() {
    const t = this._formData.productOptions ?? [], e = s(this, h)?.isNew ?? !0, i = at(t), a = this._optionSettings?.maxProductOptions ?? 5, r = t.length >= a;
    return o`
      <div class="tab-content">
        ${e ? o`
      ...
      <div class="option-value-chip">
        ...
        ${i ? o`
              <span class="adjustments">
                ${t.priceAdjustment !== 0 ? o`<span class="price-adjustment">${t.priceAdjustment > 0 ? "+" : ""}${C(t.priceAdjustment)}</span>` : l}
                ${t.priceAdjustment !== 0 && t.costAdjustment !== 0 ? o`<span class="adjustment-separator">/</span>` : l}
                ${t.costAdjustment !== 0 ? o`<span class="cost-adjustment">${t.costAdjustment > 0 ? "+" : ""}${C(t.costAdjustment)} cost</span>` : l}
              </span>
            ` : l}
      </div>
    `;
  ...
  async _deleteOption(t) {
    const i = this._formData.productOptions?.find((n) => n.id === t)?.name || "this option", a = s(this, b)?.open(this, P, {
      data: {
  ...
  async _saveOptions() {
    ...
    try {
      ...
      }), s(this, h)?.reload()) : r && (this._errorMessage = "Failed to save options: " + r.message, s(this, g)?.peek("danger", { data: { headline: "Failed to save options", message: r.message } }));
    } catch (e) {
      if (!s(this, _)) return;
...
//# sourceMappingURL=product-detail.element-D_CZbWu5.js.map
