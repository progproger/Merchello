import { html as n, nothing as l, css as D, state as p, customElement as T } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalToken as k, UmbModalBaseElement as z, UMB_MODAL_MANAGER_CONTEXT as R } from "@umbraco-cms/backoffice/modal";
import { UMB_NOTIFICATION_CONTEXT as A } from "@umbraco-cms/backoffice/notification";
import { M as y } from "./merchello-api-DFeoGYDY.js";
...
import { c as a } from "./formatting-DPh4-DfL.js";
...
var q = Object.defineProperty, E = Object.getOwnPropertyDescriptor, S = (e) => {
  ...
  for (var s = t > 1 ? void 0 : t ? E(i, o) : i, r = e.length - 1, g; r >= 0; r--)
    (g = e[r]) && (s = (t ? g(i, o, s) : g(s)) || s);
  return t && s && q(i, o, s), s;
}, I = (e, i, o) => i.has(e) || S("Cannot " + o), v = (e, i, o) => (I(e, i, "read from private field"), i.get(e)), x = (e, i, o) => i.has(e) ? S("Cannot add the same private member more than once") : i instanceof WeakSet ? i.add(e) : i.set(e, o), $ = (e, i, o, t) => (I(e, i, "write to private field"), i.set(e, o), o), h, _;
let d = class extends z {
  constructor() {
    super(), this._invoice = null, this._isLoading = !0, this._isSaving = !1, this._errorMessage = null, this._editReason = "", this._orders = [], this._lineItems = [], this._customItems = [], this._pendingProducts = [], this._taxGroups = [], this._removedShippingOrders = /* @__PURE__ */ new Set(), this._taxRemoved = !1, this._removedOrderDiscounts = /* @__PURE__ */ new Set(), this._pendingOrderDiscounts = [], this._pendingOrderDiscountCodes = [], this._previewResult = null, this._previewLoading = !1, x(this, h), x(this, _), this.consumeContext(R, (e) => {
      $(this, h, e);
  ...
  async _loadInvoice() {
    ...
    this._invoice = e.data ?? null, this._taxGroups = i.data ?? [], this._invoice && (this._orders = this._invoice.orders.map((o) => ({
      ...
      (o) => o.lineItems.map((t) => {
        const s = t.discounts.length > 0, r = s ? {
  ...
  _updateQuantity(e, i) {
    this._lineItems = this._lineItems.map((o) => o.id === e ? {
      ...o,
  ...
  async _openLineItemDiscountModal(e) {
    ...
    if (!i) return;
    const t = await v(this, h).open(this, w, {
      data: {
  ...
  async _openAddDiscountModal() {
    if (!v(this, h) || !this._invoice) return;
    const i = await v(this, h).open(this, w, {
      data: {
        currencySymbol: this._invoice.currencySymbol,
        currencyCode: this._invoice.currencyCode,
        isOrderDiscount: !0
    ...
    if (i?.discount && (this._pendingOrderDiscounts = [
      ...
      const o = i.discountCode.trim();
      if (!o) return;
      if (this._pendingOrderDiscountCodes.some(
        (s) => s.code.toLowerCase() === o.toLowerCase()
      )) {
        v(this, _)?.peek("warning", {
          data: {
            headline: "Discount Already Added",
            message: `Discount code '${o}' is already pending.`
          }
        });
        return;
      }
  ...
  _getLineItemCalculatedTotal(e) {
    return this._previewResult ? this._previewResult.lineItems.find((o) => o.id === e)?.calculatedTotal ?? null : null;
  }
  /**
  ...
  _getDiscountedUnitPriceFromPreview(e) {
    return this._previewResult ? this._previewResult.lineItems.find((o) => o.id === e)?.discountedUnitPrice ?? null : null;
  }
  async _openAddCustomItemModal() {
    if (!v(this, h) || !this._invoice) return;
    const i = await v(this, h).open(this, L, {
      data: {
        currencySymbol: this._invoice.currencySymbol,
  ...
  async _openProductPickerModal() {
    if (!v(this, h) || !this._invoice) return;
    const e = this._lineItems.filter((s) => s.productId && !s.isRemoved).map((s) => s.productId), i = this._pendingProducts.map((s) => s.productId), t = await v(this, h).open(this, M, {
      data: {
        config: {
          currencySymbol: this._invoice.currencySymbol,
  ...
  _refreshPreview() {
    ...
    this._previewDebounceTimer && clearTimeout(this._previewDebounceTimer), this._previewDebounceTimer = setTimeout(async () => {
      ...
      const i = this._buildPreviewRequest(), { data: o, error: t } = await y.previewInvoiceEdit(e, i);
      this._previewLoading = !1, !t && o && (this._previewResult = o, this._updateLineItemTotalsFromPreview(o));
    }, 300);
  }
  /**
  ...
  _renderLineItem(e) {
    ...
    const i = this._invoice?.currencySymbol ?? "£", o = e.discount !== null, t = this._previewResult?.lineItems.find((m) => m.id === e.id), s = t?.hasInsufficientStock ?? !1, r = t?.canAddDiscount ?? !0, g = e.newQuantity < e.quantity, f = e.childLineItems ?? [];
    return n`
      <div class="line-item ${s ? "has-error" : ""} ${f.length > 0 ? "has-addons" : ""}">
        <div class="line-item-product">
          ...
          ${e.isStockTracked && e.availableStock !== null ? n`
            <div class="stock-info ${s ? "error" : ""}">
              ${s ? n`<uui-icon name="icon-alert"></uui-icon> Only ${e.availableStock} available` : n`${e.availableStock} in stock`}
            </div>
          ` : l}
        </div>
      ...
      <div class="line-item child-item addon-item">
        <div class="line-item-product">
          ...
          <div class="line-item-details">
            ...
            ${e.sku ? n`<div class="line-item-sku">${e.sku}</div>` : l}
          </div>
        </div>
        ...
        <div class="line-item-total">
          ${t !== null ? n`+${o}${a(t, 2)}` : n`<span class="calculating">...</span>`}
        </div>
  ...
  _renderOrderDiscounts() {
    ...
    return n`
            ...
            <div class="order-discount-row">
              <div class="discount-info">
                ...
                <span class="discount-value">
                  ${s.type === b.Percentage ? `${s.value}%` : `${t}${a(s.value, 2)}`}
                </span>
              </div>
      ...
      (s) => n`
            <div class="order-discount-row pending">
              <div class="discount-info">
                <span class="discount-name">${s.name || "Discount code"}</span>
                <span class="discount-value code">${s.code}</span>
              ...
              <div class="discount-amount-cell">
                <uui-button
                  ...
                  look="secondary"
                  color="danger"
                  @click=${() => this._removePendingOrderDiscountCode(s.tempId)}
  ...
  _renderCustomItem(e) {
    ...
    return n`
      ...
      ${(e.addons ?? []).map((s) => this._renderPendingCustomItemAddon(s, e.quantity, i))}
    `;
  }
  _renderPendingCustomItemAddon(e, i, o) {
    return n`
      <div class="line-item child-item addon-item">
        <div class="line-item-product">
          <div class="addon-indicator">
  ...
  _renderPendingProduct(e) {
    ...
    return n`
      ...
      ${e.addons.map((o) => this._renderPendingAddon(o, e.quantity, i))}
    `;
  }
  ...
  render() {
    ...
    const e = this._invoice.currencySymbol, i = this._invoice.currencyCode, o = this._subtotalBeforeDiscounts, t = this._discountTotal, s = this._adjustedSubtotal, r = this._newTax, g = this._newTotal, f = this._hasChanges(), m = this._lineItems.filter((u) => u.isRemoved), C = t > 0;
    return n`
              ...
              <div class="payment-row shipping-row ${this._removedShippingOrders.has(u.id) ? "removed" : ""}">
                ...
                <div class="summary-edit-controls">
                  ${this._removedShippingOrders.has(u.id) ? n`
                    ...
                    <uui-button compact look="secondary" label="Undo" @click=${() => this._restoreShipping(u.id)}>Undo</uui-button>
                  ` : n`
                    <div class="summary-input">
                      ...
                      <uui-input
                        ...
                        @input=${(O) => this._updateOrderShipping(u.id, parseFloat(O.target.value) || 0)}
                        min="0"
                        step="0.01"
                      ></uui-input>
                    </div>
                    <uui-button compact look="secondary" label="Remove shipping" @click=${() => this._removeShipping(u.id)} title="Remove shipping">
                      <uui-icon name="icon-delete"></uui-icon>
                    </uui-button>
                  `}
                </div>
...
c([
  p()
], d.prototype, "_pendingOrderDiscountCodes", 2);
c([
  p()
...
//# sourceMappingURL=edit-order-modal.element-C1GmvY-r.js.map
