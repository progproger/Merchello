import { html as t, nothing as u, css as S, state as c, customElement as w } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalToken as $, UmbModalBaseElement as C, UMB_MODAL_MANAGER_CONTEXT as O, UMB_CONFIRM_MODAL as x } from "@umbraco-cms/backoffice/modal";
...
import { m as M } from "./modal-layout.styles-C2OaUji5.js";
const f = new $("Merchello.TaxGroup.Modal", {
  modal: {
    type: "sidebar",
    size: "medium"
  }
...
var z = Object.defineProperty, L = Object.getOwnPropertyDescriptor, T = (e) => {
  ...
  for (var o = s > 1 ? void 0 : s ? L(i, a) : i, b = e.length - 1, m; b >= 0; b--)
    (m = e[b]) && (o = (s ? m(i, a, o) : m(o)) || o);
  return s && o && z(i, a, o), o;
}, k = (e, i, a) => i.has(e) || T("Cannot " + a), r = (e, i, a) => (k(e, i, "read from private field"), i.get(e)), _ = (e, i, a) => i.has(e) ? T("Cannot add the same private member more than once") : i instanceof WeakSet ? i.add(e) : i.set(e, a), v = (e, i, a, s) => (k(e, i, "write to private field"), i.set(e, a), a), n, p, h;
let l = class extends C {
  constructor() {
    super(), this._fields = [], this._values = {}, this._isLoading = !0, this._isSaving = !1, this._errorMessage = null, this._visibleSecrets = /* @__PURE__ */ new Set(), this._activeTab = "product", this._taxGroups = [], this._isLoadingTaxGroups = !1, this._isDeleting = null, this._shippingOverrides = [], this._isLoadingOverrides = !1, this._deletedOverrideIds = [], this._mappings = {}, _(this, n, !1), _(this, p), _(this, h), this.consumeContext(O, (e) => {
      v(this, p, e);
  ...
  connectedCallback() {
    super.connectedCallback(), v(this, n, !0), this._loadData();
  }
  ...
  async _loadData() {
    ...
    this._values = {};
    for (const o of this._fields)
      this._values[o.key] = s[o.key] ?? o.defaultValue ?? "";
    if (this._isLoading = !1, this._isManualProvider && await Promise.all([this._loadTaxGroups(), this._loadShippingOverrides()]), !this._isManualProvider && this._fields.some((o) => o.fieldType === "TaxGroupMapping")) {
      await this._loadTaxGroups();
      const o = this._values.taxGroupMappings;
      if (o)
        try {
          this._mappings = JSON.parse(o);
        } catch {
          this._mappings = {};
        }
    }
  ...
  async _handleAddTaxGroup() {
    const i = await r(this, p)?.open(this, f, {
      data: {}
    })?.onSubmit().catch(() => {
    });
    r(this, n) && i?.isCreated && (r(this, h)?.peek("positive", {
      data: {
  ...
  async _handleEditTaxGroup(e) {
    const a = await r(this, p)?.open(this, f, {
      data: { taxGroup: e }
    })?.onSubmit().catch(() => {
    });
    r(this, n) && a?.isUpdated && (r(this, h)?.peek("positive", {
      data: {
  ...
  async _handleDeleteTaxGroup(e, i) {
    ...
    const a = r(this, p)?.open(this, x, {
      data: {
        headline: "Delete Tax Group",
        content: `Delete tax group "${i.name}". Products using this tax group must be reassigned.`,
        confirmLabel: "Delete",
        color: "danger"
      }
    });
    ...
    const { error: s } = await g.deleteTaxGroup(i.id);
    if (r(this, n)) {
      if (this._isDeleting = null, s) {
        r(this, h)?.peek("danger", {
          data: { headline: "Failed to delete", message: s.message || "Could not delete tax group" }
        });
        return;
      }
  ...
  async _handleAddOverride() {
    const i = await r(this, p)?.open(this, y, {
      data: {}
    })?.onSubmit().catch(() => {
    });
    r(this, n) && i?.isSaved && this._loadShippingOverrides();
  }
  async _handleEditOverride(e) {
    const a = await r(this, p)?.open(this, y, {
      data: { override: e }
    })?.onSubmit().catch(() => {
    });
    r(this, n) && a?.isSaved && this._loadShippingOverrides();
  }
  async _handleDeleteOverride(e, i) {
    e.preventDefault(), e.stopPropagation();
    const a = i.regionName ? `${i.countryName} - ${i.regionName}` : i.countryName || i.countryCode, s = r(this, p)?.open(this, x, {
      data: {
        headline: "Delete Shipping Tax Override",
        content: `Delete the shipping tax override for ${a}.`,
        confirmLabel: "Delete",
        color: "danger"
      }
    });
    ...
    r(this, n) && (this._deletedOverrideIds = [...this._deletedOverrideIds, i.id], this._shippingOverrides = this._shippingOverrides.filter((o) => o.id !== i.id));
  }
  ...
  _handleMappingChange(e, i) {
    if (i)
      this._mappings = { ...this._mappings, [e]: i };
    else {
      const { [e]: a, ...s } = this._mappings;
      this._mappings = s;
    }
    this._values = { ...this._values, taxGroupMappings: JSON.stringify(this._mappings) };
  }
  _renderTaxGroupMappingField(e) {
    return this._isLoadingTaxGroups ? t`
      ...
      (i) => t`
                <uui-table-row>
                  <uui-table-cell>
                    <span class="name-cell">${i.name}</span>
                    <span class="rate-cell">(${i.taxPercentage}%)</span>
                  </uui-table-cell>
                  <uui-table-cell>
                    <uui-input
                      label="Tax code for ${i.name}"
                      .value=${this._mappings[i.id] ?? ""}
                      @input=${(a) => this._handleMappingChange(i.id, a.target.value)}
                      placeholder="${e.placeholder || "P0000000"}"
                    ></uui-input>
                  </uui-table-cell>
                </uui-table-row>
              `
      ...
      try {
        ...
        if (this._isManualProvider && this._deletedOverrideIds.length > 0) {
          for (const a of this._deletedOverrideIds)
            (await g.deleteShippingTaxOverride(a)).error;
  ...
  _formatRegion(e) {
    return e.regionName && e.countryName ? `${e.countryName} - ${e.regionName}` : e.countryName ? e.countryName : e.regionCode ? `${e.countryCode}-${e.regionCode}` : e.countryCode;
  }
  // Render: Config field
  _renderField(e) {
    ...
    switch (e.fieldType) {
      ...
      case "Url":
        ...
        `;
      case "Password": {
        const a = this._visibleSecrets.has(e.key);
        return t`
          <div class="form-field">
            <label for="${e.key}">${e.label}${e.isRequired ? " *" : ""}</label>
            ${e.description ? t`<p class="field-description">${e.description}</p>` : u}
            <div class="password-field-wrapper">
              ...
              >
                <uui-icon name="${a ? "icon-eye-slash" : "icon-eye"}"></uui-icon>
              </uui-button>
            </div>
      ...
      case "TaxGroupMapping":
        return this._renderTaxGroupMappingField(e);
      default:
        return u;
    }
  }
  ...
  _renderShippingTaxesTab() {
    const e = this._values.isShippingTaxable === "true", i = this._values.shippingTaxGroupId ?? "", a = [
      ...
      ...this._taxGroups.map((s) => ({
        name: `${s.name} (${s.taxPercentage}%)`,
        value: s.id,
        selected: s.id === i
      }))
    ];
        ...
        <div class="section">
          ...
          <div class="form-field checkbox-field">
            <uui-checkbox
              label="Tax shipping"
              ?checked=${e}
              @change=${(s) => this._handleCheckboxChange("isShippingTaxable", s.target.checked)}
            >
              Tax Shipping
            </uui-checkbox>
                ...
                <div class="form-field">
                  ...
                  ${this._isLoadingTaxGroups ? t`<uui-loader-circle></uui-loader-circle>` : t`
                        <uui-select
                          ...
                          .options=${a}
                          @change=${(s) => this._handleValueChange("shippingTaxGroupId", s.target.value)}
                        ></uui-select>
                      `}
                </div>
              ` : u}
                ...
                ` : t`
                  <div class="table-container">
                    <uui-table class="data-table">
                      ...
                      ${this._shippingOverrides.map((s) => this._renderOverrideRow(s))}
                    </uui-table>
                  </div>
                `}
        </div>
      ...
      <uui-table-row>
        <uui-table-cell>
          <div class="region-cell">
            ...
            ${e.regionCode ? u : t`<span class="country-badge">Country-wide</span>`}
          </div>
        </uui-table-cell>
        <uui-table-cell>
...
d([
  c()
], l.prototype, "_mappings", 2);
l = d([
  w("merchello-tax-provider-config-modal")
], l);
const A = l;
export {
...
//# sourceMappingURL=tax-provider-config-modal.element-CDiR478w.js.map
