import { LitElement as D, html as s, nothing as c, css as x, state as u, customElement as O } from "@umbraco-cms/backoffice/external/lit";
...
import { UmbModalToken as $, UMB_MODAL_MANAGER_CONTEXT as k, UMB_CONFIRM_MODAL as y } from "@umbraco-cms/backoffice/modal";
import { UMB_NOTIFICATION_CONTEXT as I } from "@umbraco-cms/backoffice/notification";
import { M as m } from "./merchello-api-DFeoGYDY.js";
import { M as A } from "./supplier-modal.token-CWeQ_zlc.js";
import { M } from "./product-picker-modal.token-BfbHsSHl.js";
import { m as E, A as L, B as P } from "./navigation-BGhEgega.js";
import { b as z } from "./badge.styles-DUcdl6GY.js";
import { a as F } from "./formatting-DPh4-DfL.js";
import "./product-table.element-Bb99ITRP.js";
import "./pagination.element-sDi4Myhy.js";
const W = new $("Merchello.ServiceRegion.Modal", {
  modal: {
    type: "sidebar",
    size: "medium"
  }
}), N = new $("Merchello.ShippingOption.Detail.Modal", {
  modal: {
    type: "sidebar",
    size: "large"
  }
});
var U = Object.defineProperty, q = Object.getOwnPropertyDescriptor, C = (e) => {
  throw TypeError(e);
}, n = (e, t, a, d) => {
  for (var l = d > 1 ? void 0 : d ? q(t, a) : t, f = e.length - 1, w; f >= 0; f--)
    (w = e[f]) && (l = (d ? w(t, a, l) : w(l)) || l);
  return d && l && U(t, a, l), l;
}, S = (e, t, a) => t.has(e) || C("Cannot " + a), i = (e, t, a) => (S(e, t, "read from private field"), t.get(e)), _ = (e, t, a) => t.has(e) ? C("Cannot add the same private member more than once") : t instanceof WeakSet ? t.add(e) : t.set(e, a), v = (e, t, a, d) => (S(e, t, "write to private field"), t.set(e, a), a), b, p, g, h, r;
function H() {
  return {
    name: null,
    email: null,
...
let o = class extends R(D) {
  ...
  connectedCallback() {
    super.connectedCallback(), v(this, r, !0), this._createRoutes(), this._loadSuppliers(), this._loadCountries(), this._loadFulfilmentProviders();
  }
  disconnectedCallback() {
    super.disconnectedCallback(), v(this, r, !1), i(this, b) && (clearTimeout(i(this, b)), v(this, b, null));
  }
  // Tab routing
  _createRoutes() {
  ...
  async _loadSuppliers() {
    ...
    i(this, r) && e && (this._suppliers = e);
  }
  async _loadCountries() {
    const { data: e } = await m.getLocalityCountries();
    i(this, r) && e && (this._countries = e);
  }
  async _loadFulfilmentProviders() {
    const { data: e } = await m.getFulfilmentProviderOptions();
    i(this, r) && e && (this._fulfilmentProviderOptions = e);
  }
  async _loadShippingOptions() {
    if (!this._warehouse?.id) return;
    const { data: e } = await m.getShippingOptions();
    i(this, r) && e && (this._shippingOptions = e.filter((t) => t.warehouseId === this._warehouse?.id));
  }
  _handleInputChange(e, t) {
    this._formData = { ...this._formData, [e]: t };
  }
  _handleAddressChange(e, t) {
    const a = this._formData.address ?? H();
    this._formData = {
      ...this._formData,
      address: {
        ...a,
  ...
  async _openCreateSupplierModal() {
    if (!i(this, g)) return;
    const t = await i(this, g).open(this, A, {
      data: {}
    }).onSubmit().catch(() => {
    });
    i(this, r) && t?.isCreated && t.supplier && (this._suppliers = [...this._suppliers, t.supplier], this._formData = { ...this._formData, supplierId: t.supplier.id });
  }
  async _handleSave() {
    ...
    try {
      if (e) {
        const { data: t, error: a } = await m.createWarehouse({
        ...
        t && (i(this, p)?.updateWarehouse(t), history.replaceState({}, "", E(t.id)), i(this, h)?.peek("positive", {
          data: { headline: "Warehouse created", message: "The warehouse has been created successfully" }
        }));
      } else {
        const { data: t, error: a } = await m.updateWarehouse(this._warehouse.id, {
          name: this._formData.name,
          code: this._formData.code,
          supplierId: this._formData.supplierId,
      ...
      i(this, r) && (this._isSaving = !1);
    }
  }
  async _handleDelete() {
    if (!this._warehouse?.id) return;
    const e = i(this, g)?.open(this, y, {
      data: {
    ...
    if (!i(this, r)) return;
    const { error: t } = await m.deleteWarehouse(this._warehouse.id);
    if (i(this, r)) {
      if (t) {
  ...
  async _openServiceRegionModal(e) {
    if (!i(this, g) || !this._warehouse?.id) return;
    const a = await i(this, g).open(this, W, {
      data: {
    ...
    i(this, r) && a?.isSaved && i(this, p)?.reload();
  }
  async _handleDeleteRegion(e, t) {
    if (e.stopPropagation(), !this._warehouse?.id) return;
    const a = t.regionDisplay || `${t.regionCode || ""} ${t.countryCode}`.trim(), d = i(this, g)?.open(this, y, {
      data: {
    ...
    const { error: l } = await m.deleteServiceRegion(this._warehouse.id, t.id);
    if (i(this, r)) {
      if (this._isDeletingRegion = null, l) {
        this._errorMessage = `Failed to delete region: ${l.message}`, i(this, h)?.peek("danger", {
          data: { headline: "Failed to delete", message: l.message || "Could not delete region" }
        });
        return;
      }
      i(this, h)?.peek("positive", {
  ...
  async _openShippingOptionModal(e) {
    if (!i(this, g) || !this._warehouse?.id) return;
    const a = await i(this, g).open(this, N, {
      data: {
        optionId: e?.id,
        warehouseId: this._warehouse.id
      }
    ...
    i(this, r) && a?.isSaved && (this._loadShippingOptions(), i(this, p)?.reload());
  }
  async _handleDeleteOption(e, t) {
    e.stopPropagation();
    const a = i(this, g)?.open(this, y, {
      data: {
        headline: "Delete Shipping Option",
        content: `Are you sure you want to delete shipping option "${t.name}"?`,
        confirmLabel: "Delete",
        color: "danger"
      }
    });
    ...
    const { error: d } = await m.deleteShippingOption(t.id);
    if (i(this, r)) {
  ...
  async _loadWarehouseProducts() {
    ...
    const { data: e, error: t } = await m.getWarehouseProducts(
      this._warehouse.id,
      this._productsPage,
      20,
  ...
  _handleProductsSearchInput(e) {
    ...
    i(this, b) && clearTimeout(i(this, b)), v(this, b, setTimeout(() => {
  ...
  async _handleAddProducts() {
    if (!i(this, g) || !this._warehouse?.id) return;
    const e = this._warehouseProducts.map((d) => d.productRootId), a = await i(this, g).open(this, M, {
      data: {
        config: {
          currencySymbol: "£",
    ...
    if (i(this, r) && a?.selections?.length) {
      const d = [...new Set(a.selections.map((f) => f.productRootId))], { error: l } = await m.addProductsToWarehouse(this._warehouse.id, d);
      if (!i(this, r)) return;
      if (l) {
        i(this, h)?.peek("danger", {
          data: { headline: "Failed to add products", message: l.message }
        });
        return;
      }
      i(this, h)?.peek("positive", {
  ...
  async _handleRemoveSelectedProducts() {
    ...
    const { error: t } = await m.removeProductsFromWarehouse(
      this._warehouse.id,
  ...
  _getValidationClass(e, t) {
    return t && this._isFieldEmpty(e) ? "field-warning" : "";
  }
  // Render methods
  _renderLoadingState() {
  ...
  _renderOptionRow(e) {
    ...
    let l;
    return a ? l = "Live Rates" : e.fixedCost != null ? l = F(e.fixedCost) : e.costCount > 0 ? l = `${e.costCount} location(s)` : l = "—", s`
      <uui-table-row class="clickable" @click=${() => this._openShippingOptionModal(e)}>
        ...
        <uui-table-cell class="actions-cell" @click=${(f) => f.stopPropagation()}>
          <uui-button
            look="secondary"
            compact
            label="Edit"
          ...
          <uui-button
            ...
            label="Delete"
            ?disabled=${t}
            @click=${(f) => this._handleDeleteOption(f, e)}>
            <uui-icon name="${t ? "icon-hourglass" : "icon-trash"}"></uui-icon>
          </uui-button>
        </uui-table-cell>
      </uui-table-row>
  ...
  _renderProductsTab() {
    ...
    return s`
      <div class="tab-content">
        ...
        ${this._isLoadingProducts ? s`<div class="loading"><uui-loader></uui-loader></div>` : t.length > 0 ? s`
  ...
  render() {
    ...
    return s`
      <umb-body-layout header-fit-height main-no-padding>
        <!-- Back button -->
        <uui-button slot="header" compact href=${P()} label="Back to Warehouses" class="back-button">
        ...
        <umb-footer-layout slot="footer">
          <uui-breadcrumbs>
            <uui-breadcrumb-item href=${P()}>Warehouses</uui-breadcrumb-item>
...
//# sourceMappingURL=warehouse-detail.element-CllGJhoJ.js.map
