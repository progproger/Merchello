import { LitElement as J, nothing as u, html as o, css as X, state as g, customElement as Q } from "@umbraco-cms/backoffice/external/lit";
...
import { e as G } from "./formatting-CZRy3TEt.js";
import "./merchello-empty-state.element-mt97UoA5.js";
var se = Object.defineProperty, oe = Object.getOwnPropertyDescriptor, q = (e) => {
  throw TypeError(e);
}, m = (e, t, i, r) => {
  for (var s = r > 1 ? void 0 : r ? oe(t, i) : t, a = e.length - 1, l; a >= 0; a--)
    (l = e[a]) && (s = (r ? l(t, i, s) : l(s)) || s);
  return r && s && se(t, i, s), s;
}, V = (e, t, i) => t.has(e) || q("Cannot " + i), d = (e, t, i) => (V(e, t, "read from private field"), t.get(e)), E = (e, t, i) => t.has(e) ? q("Cannot add the same private member more than once") : t instanceof WeakSet ? t.add(e) : t.set(e, i), k = (e, t, i, r) => (V(e, t, "write to private field"), t.set(e, i), i), w, f, I, S;
const T = 5;
let p = class extends K(J) {
  constructor() {
    super(), this._isNew = !0, this._isLoading = !0, this._isSaving = !1, this._isRebuilding = !1, this._isPreviewLoading = !1, this._loadError = null, this._validationErrors = {}, this._productTypes = [], this._collections = [], this._filterGroups = [], this._resolvers = [], this._countries = [], this._routes = [], this._activePath = "", this._customLabelArgsText = Array.from({ length: T }, () => "{}"), this._customLabelArgsErrors = Array.from({ length: T }, () => ""), this._customFieldArgsText = [], this._customFieldArgsErrors = [], this._isSlugManuallyEdited = !1, E(this, w), E(this, f), E(this, I), E(this, S, !1), this._initRoutes(), this.consumeContext(Y, (e) => {
      k(this, w, e), d(this, w) && (this._isNew = d(this, w).isNew, this.observe(d(this, w).feed, (t) => {
        t && this._applyFeed(t);
      }, "_feed"), this.observe(d(this, w).isLoading, (t) => {
        this._isLoading = t;
      }, "_isLoading"), this.observe(d(this, w).loadError, (t) => {
        this._loadError = t;
      }, "_loadError"));
  ...
  connectedCallback() {
    super.connectedCallback(), k(this, S, !0), this._loadReferenceData();
  }
  disconnectedCallback() {
    super.disconnectedCallback(), k(this, S, !1);
  }
  _initRoutes() {
  ...
  async _loadReferenceData() {
    const [e, t, i, r, s] = await Promise.all([
      C.getProductTypes(),
      C.getProductCollections(),
      C.getFilterGroups(),
      C.getProductFeedResolvers(),
      C.getCountries()
    ]);
    d(this, S) && (e.data && (this._productTypes = e.data), t.data && (this._collections = t.data), i.data && (this._filterGroups = i.data), r.data && (this._resolvers = r.data), s.data && (this._countries = s.data));
  }
  ...
  _normalizeFeed(e) {
    ...
    for (const r of e.customLabels ?? [])
      r.slot < 0 || r.slot >= T || t.has(r.slot) || t.set(r.slot, {
        slot: r.slot,
    ...
    const i = Array.from({ length: T }, (r, s) => t.get(s) ?? this._createEmptyCustomLabel(s));
    return {
      ...e,
  ...
  _applyFeed(e) {
    ...
    this._feed = t, this._isLoading = !1, this._isNew = !t.id, this._loadError = null, this._validationErrors = {}, this._customLabelArgsText = t.customLabels.map((i) => this._formatArgs(i.args)), this._customLabelArgsErrors = Array.from({ length: T }, () => ""), this._customFieldArgsText = t.customFields.map((i) => this._formatArgs(i.args)), this._customFieldArgsErrors = t.customFields.map(() => ""), this._isSlugManuallyEdited = this._isSlugOverride(t.name, t.slug);
  }
  _commitFeed(e) {
    this._feed = e;
  }
  ...
  _toggleFilterValue(e, t, i, r) {
    const s = e.filterValueGroups.find((h) => h.filterGroupId === t);
    let a = e.filterValueGroups.filter((h) => h.filterGroupId !== t);
  ...
  _buildSelectionQueryInfo(e) {
    const t = this._resolveNamesByIds(this._productTypes, e.productTypeIds), i = this._resolveNamesByIds(this._collections, e.collectionIds), r = e.filterValueGroups.map((n) => {
      const y = this._filterGroups.find(($) => $.id === n.filterGroupId), _ = this._resolveNamesByIds(y?.filters ?? [], n.filterIds);
      return {
        groupName: y?.name ?? n.filterGroupId,
        valueNames: _
      };
    }).filter((n) => n.valueNames.length > 0), s = [];
    t.length > 0 && s.push(`productType IN (${t.map((n) => `"${n}"`).join(", ")})`), i.length > 0 && s.push(`collection IN (${i.map((n) => `"${n}"`).join(", ")})`);
    for (const n of r)
      s.push(`${n.groupName} IN (${n.valueNames.map((y) => `"${y}"`).join(", ")})`);
    ...
    for (const n of r)
      l.push(`${n.groupName} is ${this._formatSentenceValueList(n.valueNames)}`);
    const h = l.length > 0 ? `Include products where ${l.join(" and ")}.` : "Include all products. No product type, collection, or filter-value restrictions are active.";
    return {
      productTypeNames: t,
      collectionNames: i,
      filterGroups: r,
  ...
  _reparseArgsForSave() {
    ...
    for (let r = 0; r < T; r++) {
      const s = this._feed.customLabels.find((h) => h.slot === r);
  ...
  async _handleSave() {
    ...
    if (this._isNew) {
      const { data: r, error: s } = await C.createProductFeed(e);
      if (this._isSaving = !1, s || !r) {
        d(this, f)?.peek("danger", {
          data: {
      ...
      this._isNew = !1, this._applyFeed(r), d(this, w)?.updateFeed(r), d(this, f)?.peek("positive", {
        data: {
    ...
    const { data: t, error: i } = await C.updateProductFeed(this._feed.id, e);
    ...
    this._applyFeed(t), d(this, w)?.updateFeed(t), d(this, f)?.peek("positive", {
      data: {
        headline: "Feed saved",
  ...
  async _handleDelete() {
    ...
    const { error: t } = await C.deleteProductFeed(this._feed.id);
    if (t) {
      d(this, f)?.peek("danger", {
        data: {
  ...
  async _reloadCurrentFeed() {
    ...
    const { data: e } = await C.getProductFeed(this._feed.id);
    !d(this, S) || !e || (this._applyFeed(e), d(this, w)?.updateFeed(e));
  }
  async _handleRebuild() {
    if (!this._feed?.id || this._isNew) return;
    this._isRebuilding = !0;
    const { data: e, error: t } = await C.rebuildProductFeed(this._feed.id);
    if (this._isRebuilding = !1, !!d(this, S)) {
      if (t || !e) {
        d(this, f)?.peek("danger", {
          data: {
      ...
      this._lastRebuild = e, e.success ? d(this, f)?.peek("positive", {
        data: {
  ...
  async _handlePreview() {
    ...
    const { data: e, error: t } = await C.previewProductFeed(this._feed.id);
    if (this._isPreviewLoading = !1, !!d(this, S)) {
      if (t || !e) {
        d(this, f)?.peek("danger", {
          data: {
      ...
      this._preview = e, e.error && d(this, f)?.peek("warning", {
        data: {
  ...
  async _copyToClipboard(e, t) {
    try {
      await navigator.clipboard.writeText(e), d(this, f)?.peek("positive", {
        data: {
          headline: "Copied",
          message: t
        }
  ...
  _renderSelectionChecklist(e, t, i, r, s, a, l) {
    const h = this._productTypes.length, n = e.productTypeIds.length, y = h > 0 && n === h, _ = n > 0 && !y, $ = n === 0 ? `0 of ${h} selected • No restriction (all product types)` : y ? `${n} of ${h} selected • Effectively no restriction` : `${n} of ${h} selected • OR logic (any selected type)`, v = this._collections.length, b = e.collectionIds.length, M = v > 0 && b === v, L = b > 0, U = b === 0 ? `0 of ${v} selected • No restriction (all collections + unassigned)` : M ? `${b} of ${v} selected • OR logic (products must be in at least one collection)` : `${b} of ${v} selected • OR logic (any selected collection)`, N = e.filterValueGroups.filter((c) => c.filterIds.length > 0), D = N.reduce((c, x) => c + x.filterIds.length, 0), z = N.length > 0, A = this._buildSelectionQueryInfo(e);
    return o`
      ...
      <uui-box headline="Selection Logic">
        <div class="logic-grid">
          <section class="logic-rule">
            <div class="logic-rule-header">
              ...
              <uui-tag color=${_ ? "warning" : "positive"}>
                ${_ ? "Active • OR" : "No restriction"}
              </uui-tag>
            </div>
        ...
        <div class="logic-summary">
          ...
          <ul class="logic-list">
            <li>Product Types: ${_ ? "restricted" : "not restricted"}</li>
          ...
          <div class="query-groups">
            <section class="query-group">
              ...
              <div class="query-tag-list">
                ${A.productTypeNames.length > 0 ? A.productTypeNames.map((c) => o`<uui-tag color="warning">${c}</uui-tag>`) : o`<uui-tag color="positive">Any</uui-tag>`}
              </div>
            </section>
            ...
            <section class="query-group">
              ...
              <div class="query-tag-list">
                ${A.collectionNames.length > 0 ? A.collectionNames.map((c) => o`<uui-tag color="warning">${c}</uui-tag>`) : o`<uui-tag color="positive">Any</uui-tag>`}
              </div>
            </section>

            <section class="query-group full-width">
              <strong>Filter Values</strong>
              ${A.filterGroups.length === 0 ? o`
                    <div class="query-tag-list">
                      <uui-tag color="positive">Any</uui-tag>
                    </div>
                  ` : o`
                    <div class="query-filter-groups">
                      ${A.filterGroups.map((c) => o`
                        <div class="query-filter-group">
                          <span class="query-filter-group-name">${c.groupName}</span>
                          <div class="query-tag-list">
                            ${c.valueNames.map((x) => o`<uui-tag color="warning">${x}</uui-tag>`)}
                          </div>
                        </div>
                      `)}
                    </div>
      ...
      <uui-box headline="Product Types">
        ...
        ${this._productTypes.length === 0 ? o`<p class="hint">No product types found.</p>` : o`
              <div class="checkbox-grid">
                ${this._productTypes.map((c) => {
      const x = e.productTypeIds.includes(c.id);
      return o`
                    <uui-checkbox
                      ?checked=${x}
                      @change=${(F) => t(c.id, F.target.checked)}>
                      ${c.name}
                    </uui-checkbox>
                  `;
    })}
      ...
      <uui-box headline="Collections">
        ${v > 0 ? o`
              <div class="selection-toolbar">
                <div class="selection-meta">
                  <span class="selection-count">${U}</span>
                  <div class="selection-actions">
                    <uui-button
                      compact
                      look="secondary"
      ...
      const x = e.collectionIds.includes(c.id);
      return o`
                    <uui-checkbox
                      ?checked=${x}
                      @change=${(F) => i(c.id, F.target.checked)}>
                      ${c.name}
                    </uui-checkbox>
                  `;
    })}
      ...
      <uui-box headline="Filter Values">
        ...
        ${this._filterGroups.length === 0 ? o`<p class="hint">No filter groups found.</p>` : o`
              <div class="group-list">
                ${this._filterGroups.map((c) => {
      const x = e.filterValueGroups.find((P) => P.filterGroupId === c.id), F = this._getSelectedFilterCount(e, c.id), R = (c.filters ?? []).length, H = R > 0 && F === R, j = F === 0 ? `0 of ${R} selected • Group ignored` : `${F} of ${R} selected • OR in this group`;
      return o`
                    <section class="group-card">
                      ...
                      <div class="checkbox-grid">
                        ${(c.filters ?? []).map((P) => {
        const W = x?.filterIds.includes(P.id) ?? !1;
        return o`
                            <uui-checkbox
                              ?checked=${W}
                              @change=${(B) => r(c.id, P.id, B.target.checked)}>
                              ${P.name}
                            </uui-checkbox>
                          `;
      })}
      ...
      return o`
            ...
            <div class="grid two-col-grid">
              <umb-property-layout label="Source Type">
                <uui-select
                  ...
                  @change=${(_) => this._updateCustomLabel(e.slot, ($) => ({
        ...$,
                  ...
                  ` : o`
                    <umb-property-layout label="Static Value">
                      <uui-input
                        slot="editor"
                        .value=${e.staticValue ?? ""}
                        @input=${(_) => this._updateCustomLabel(e.slot, ($) => ({
        ...$,
        staticValue: _.target.value || null
              ...
              <div class="resolver-panel full-row">
                ${s ? o`
                      ${a ? o`<p class="hint resolver-help">${h}</p>` : o`<p class="hint">Select a resolver to view details.</p>`}
                      ...
                      ${a && l ? o`
                            <umb-property-layout label="Resolver Parameters (JSON)" class="full-row">
                              <uui-textarea
                                ...
                                .value=${r}
                                ?invalid=${!!i}
                                @input=${(_) => this._handleCustomLabelArgsInput(e.slot, _.target.value)}>
                              </uui-textarea>
  ...
  _renderCustomFieldsTab() {
    return this._feed ? o`
      ...
      const i = this._validationErrors[`customField-${t}`], r = this._validationErrors[`customFieldResolver-${t}`], s = this._customFieldArgsErrors[t], a = this._customFieldArgsText[t] ?? "{}", l = e.sourceType === "resolver", h = l && !!e.resolverAlias?.trim(), n = l && this._resolverSupportsArgs(e.resolverAlias), y = this._getResolverHelpText(e.resolverAlias), _ = this._getResolverArgsHelpText(e.resolverAlias), $ = this._getResolverArgsExample(e.resolverAlias);
      return o`
                  ...
                  <div class="grid two-col-grid">
                    <umb-property-layout label="Attribute" ?mandatory=${!0}>
                      <uui-input
                        slot="editor"
                        .value=${e.attribute}
                        @input=${(v) => this._updateCustomField(t, (b) => ({
        ...b,
        attribute: v.target.value
      }))}
                    ...
                    <umb-property-layout label="Source Type">
                      <uui-select
                        ...
                        label="Source Type"
                        .options=${this._renderSourceOptions(e.sourceType)}
                        @change=${(v) => this._updateCustomField(t, (b) => ({
        ...b,
        sourceType: v.target.value || "static"
      }))}>
                      </uui-select>
                    </umb-property-layout>
                    ...
                    ${l ? o`
                          <umb-property-layout label="Resolver" ?mandatory=${!0}>
                            <uui-select
                              ...
                              @change=${(v) => this._updateCustomField(t, (b) => ({
        ...b,
        resolverAlias: v.target.value || null
      }))}>
                            </uui-select>
                          </umb-property-layout>
                        ` : o`
                          <umb-property-layout label="Static Value">
                            <uui-input
                              ...
                              @input=${(v) => this._updateCustomField(t, (b) => ({
        ...b,
        staticValue: v.target.value || null
      }))}>
                            </uui-input>
                          </umb-property-layout>
                        `}
                    ...
                    <div class="resolver-panel">
                      ${l ? o`
                            ${h ? o`<p class="hint resolver-help">${y}</p>` : o`<p class="hint">Select a resolver to view details.</p>`}
                            ${h && n ? o`<p class="hint">This resolver supports parameters. Configure them below.</p>` : u}

                            ${h && !n ? o`<p class="hint resolver-no-args">This resolver does not take parameters.</p>` : u}
                          ` : o`<p class="hint">Static mode: this value is used for every product in this feed.</p>`}
                    </div>
                    ...
                    ${h && n ? o`
                          <umb-property-layout label="Resolver Parameters (JSON)" class="full-row">
                            <uui-textarea
                              ...
                              .value=${a}
                              ?invalid=${!!s}
                              @input=${(v) => this._handleCustomFieldArgsInput(t, v.target.value)}>
                            </uui-textarea>
                            ${s ? o`<p class="error-message">${s}</p>` : o`
                                  <p class="hint">${_}</p>
                                  <p class="hint">Example: <code>${$}</code></p>
                                `}
                          </umb-property-layout>
                        ` : u}
                  </div>
...
m([
  g()
], p.prototype, "_isSaving", 2);
m([
  g()
], p.prototype, "_isRebuilding", 2);
m([
  g()
], p.prototype, "_isPreviewLoading", 2);
m([
  g()
], p.prototype, "_loadError", 2);
m([
  g()
], p.prototype, "_validationErrors", 2);
m([
  g()
], p.prototype, "_productTypes", 2);
m([
  g()
], p.prototype, "_collections", 2);
m([
  g()
], p.prototype, "_filterGroups", 2);
m([
  g()
], p.prototype, "_resolvers", 2);
m([
  g()
...
m([
  g()
], p.prototype, "_customLabelArgsText", 2);
m([
  g()
], p.prototype, "_customLabelArgsErrors", 2);
m([
  g()
], p.prototype, "_customFieldArgsText", 2);
m([
  g()
], p.prototype, "_customFieldArgsErrors", 2);
m([
  g()
], p.prototype, "_isSlugManuallyEdited", 2);
p = m([
  Q("merchello-product-feed-detail")
], p);
const ge = p;
export {
  p as MerchelloProductFeedDetailElement,
  ge as default
};
//# sourceMappingURL=product-feed-detail.element-CixS5vVp.js.map
