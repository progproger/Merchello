import { nothing as m, html as n, css as v, state as r, customElement as f } from "@umbraco-cms/backoffice/external/lit";
...
import { b as p, g as S } from "./store-settings-7zNVo6g4.js";
import { m as x } from "./modal-layout.styles-C2OaUji5.js";
var W = Object.defineProperty, w = Object.getOwnPropertyDescriptor, _ = (i) => {
  throw TypeError(i);
}, o = (i, e, a, d) => {
  for (var u = d > 1 ? void 0 : d ? w(e, a) : e, g = i.length - 1, c; g >= 0; g--)
    (c = i[g]) && (u = (d ? c(e, a, u) : c(u)) || u);
  return d && u && W(e, a, u), u;
}, y = (i, e, a) => e.has(i) || _("Cannot " + a), l = (i, e, a) => (y(i, e, "read from private field"), e.get(i)), $ = (i, e, a) => e.has(i) ? _("Cannot add the same private member more than once") : e instanceof WeakSet ? e.add(i) : e.set(i, a), K = (i, e, a, d) => (y(i, e, "write to private field"), e.set(i, a), a), s;
let t = class extends C {
  constructor() {
    super(), this._isSaving = !1, this._isLoadingCountries = !0, this._isLoadingRegions = !1, this._countryCode = "", this._regionCode = "", this._minWeightKg = 0, this._maxWeightKg = null, this._surcharge = 0, this._currencySymbol = p(), this._countries = [], this._regions = [], $(this, s), this.consumeContext(b, (i) => {
      K(this, s, i);
    });
  }
  connectedCallback() {
    super.connectedCallback(), this._loadCurrencySymbol(), this._loadCountries(), this.data?.tier && (this._countryCode = this.data.tier.countryCode, this._regionCode = this.data.tier.regionCode ?? "", this._minWeightKg = this.data.tier.minWeightKg, this._maxWeightKg = this.data.tier.maxWeightKg ?? null, this._surcharge = this.data.tier.surcharge, this._countryCode && this._countryCode !== "*" && this._loadRegions(this._countryCode));
  }
  async _loadCurrencySymbol() {
    try {
      await S(), this._currencySymbol = p();
    } catch {
      this._currencySymbol = p();
    }
  }
  async _loadCountries() {
    if (this._isLoadingCountries = !0, this.data?.warehouseId) {
      const { data: i } = await h.getAvailableDestinationsForWarehouse(this.data.warehouseId);
      i && (this._countries = i);
    } else {
      const { data: i } = await h.getCountries();
      i && (this._countries = i);
    }
  ...
  async _loadRegions(i) {
    if (this._isLoadingRegions = !0, this._regions = [], this.data?.warehouseId) {
      const { data: e } = await h.getAvailableRegionsForWarehouse(this.data.warehouseId, i);
      e && (this._regions = e);
    } else {
      const { data: e } = await h.getLocalityRegions(i);
      e && (this._regions = e);
    }
  ...
  _handleCountryChange(i) {
    ...
    this._countryCode = e, this._regionCode = "", this._regions = [], e && e !== "*" && this._loadRegions(e);
  }
  ...
  get _regionOptions() {
    ...
    return this._regions.forEach((e) => {
      i.push({
        name: e.name,
        value: e.regionCode,
        selected: e.regionCode === this._regionCode
      });
    }), i;
  }
  async _save() {
    ...
    try {
      const e = this.data?.tier ? await h.updateShippingWeightTier(this.data.tier.id, i) : await h.addShippingWeightTier(this.data?.optionId, i);
      if (e.error) {
  ...
  render() {
    const i = !!this.data?.tier;
    return n`
      <umb-body-layout headline="${i ? "Edit" : "Add"} Weight Surcharge">
                  ...
                  ${this._isLoadingRegions ? n`<uui-loader></uui-loader>` : this._regions.length > 0 ? n`
                        ...
                        ` : n`
                          <uui-input
                            ...
                            @input=${(e) => this._regionCode = e.target.value}
      ...
      this._maxWeightKg = a ? parseFloat(a) : null;
    }}
        ...
        <div slot="actions">
          ...
          <uui-button
            look="primary"
            color="positive"
            label="${i ? "Save Surcharge" : "Add Surcharge"}"
            ?disabled=${this._isSaving || !this._countryCode}
            @click=${this._save}
          >
            ${this._isSaving ? n`<uui-loader-circle></uui-loader-circle>` : m}
            ${i ? "Save Surcharge" : "Add Surcharge"}
          </uui-button>
        </div>
      </umb-body-layout>
    `;
...
o([
  r()
], t.prototype, "_isLoadingCountries", 2);
o([
  r()
...
o([
  r()
], t.prototype, "_minWeightKg", 2);
o([
  r()
], t.prototype, "_maxWeightKg", 2);
o([
  r()
], t.prototype, "_surcharge", 2);
o([
  r()
], t.prototype, "_currencySymbol", 2);
o([
  r()
...
const R = t;
export {
  t as MerchelloShippingWeightTierModalElement,
  R as default
};
//# sourceMappingURL=shipping-weight-tier-modal.element-B3jGemOc.js.map
