...
async function r(e) {
  try {
    const t = await $(), s = u.baseUrl || "", i = await fetch(`${s}${m}/${e}`, {
      method: "GET",
...
async function o(e, t) {
  try {
    const s = await $(), i = u.baseUrl || "", a = await fetch(`${i}${m}/${e}`, {
      method: "POST",
      credentials: u.credentials,
      headers: s,
...
const w = {
  ...
  downloadCustomerStatement: async (e, t, s) => {
    try {
      ...
      const y = await l.blob(), v = l.headers.get("content-disposition");
      let f = "statement.pdf";
      if (v) {
        const h = v.match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/);
        h && h[1] && (f = h[1].replace(/['"]/g, ""));
      }
  ...
  testFulfilmentProvider: (e) => o(`fulfilment-providers/${e}/test`),
...
//# sourceMappingURL=merchello-api-DNSJzonx.js.map
