...
async function r(e) {
  try {
    const t = await v(), s = c.baseUrl || "", i = await fetch(`${s}${$}/${e}`, {
      method: "GET",
...
async function o(e, t) {
  try {
    const s = await v(), i = c.baseUrl || "", a = await fetch(`${i}${$}/${e}`, {
      method: "POST",
      credentials: c.credentials,
      headers: s,
...
const b = {
  ...
  downloadCustomerStatement: async (e, t, s) => {
    try {
      const i = await v(), a = c.baseUrl || "", d = new URLSearchParams();
      ...
      let y = "statement.pdf";
      if (f) {
        const h = f.match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/);
        h && h[1] && (y = h[1].replace(/['"]/g, ""));
      }
  ...
  validateProductImport: (e, t) => {
    ...
    return s.append("file", e), s.append("profile", String(t.profile)), t.maxIssues !== null && t.maxIssues !== void 0 && s.append("maxIssues", String(t.maxIssues)), S("product-sync/imports/validate", s);
  },
  /** Queue a product import run after validation. */
  startProductImport: (e, t) => {
    const s = new FormData();
    return s.append("file", e), s.append("profile", String(t.profile)), s.append("continueOnImageFailure", String(t.continueOnImageFailure)), t.maxIssues !== null && t.maxIssues !== void 0 && s.append("maxIssues", String(t.maxIssues)), S("product-sync/imports/start", s);
  },
...
//# sourceMappingURL=merchello-api-B1P1cUX9.js.map
