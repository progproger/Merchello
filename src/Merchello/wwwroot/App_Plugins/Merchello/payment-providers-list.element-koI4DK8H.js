import { LitElement as k, unsafeHTML as C, html as a, nothing as s, css as z, state as v, property as O, customElement as E, query as T } from "@umbraco-cms/backoffice/external/lit";
...
import { M as f } from "./merchello-api-658q9849.js";
...
var Y = Object.defineProperty, V = Object.getOwnPropertyDescriptor, S = (e) => {
  ...
  for (var n = t > 1 ? void 0 : t ? V(i, r) : i, h = e.length - 1, g; h >= 0; h--)
    (g = e[h]) && (n = (t ? g(i, r, n) : g(n)) || n);
  return t && n && Y(i, r, n), n;
}, A = (e, i, r) => i.has(e) || S("Cannot " + r), M = (e, i, r) => (A(e, i, "read from private field"), i.get(e)), X = (e, i, r) => i.has(e) ? S("Cannot add the same private member more than once") : i instanceof WeakSet ? i.add(e) : i.set(e, r), $ = (e, i, r, t) => (A(e, i, "write to private field"), i.set(e, r), r), m;
const K = {
  US: "🇺🇸",
  NL: "🇳🇱",
  BE: "🇧🇪",
...
let p = class extends L(k) {
  constructor() {
    super(...arguments), this._preview = null, this._isLoading = !0, this._errorMessage = null, this._isCollapsed = !1, this.autoLoad = !0, X(this, m, !1);
  }
  connectedCallback() {
    super.connectedCallback(), $(this, m, !0), this.autoLoad && this.loadPreview();
  }
  disconnectedCallback() {
    super.disconnectedCallback(), $(this, m, !1);
  }
  async loadPreview() {
    this._isLoading = !0, this._errorMessage = null;
    try {
      const e = await f.getCheckoutPaymentPreview();
      if (!M(this, m)) return;
      if (e.error) {
  ...
  _renderMethodIcon(e) {
    ...
    return i ? a`<span class="method-icon-svg">${C(i)}</span>` : a`<uui-icon name="${e.icon ?? "icon-credit-card"}"></uui-icon>`;
  }
  _renderRegionBadges(e) {
    return !e || e.length === 0 ? s : a`
      ${e.map(
      (i) => a`<span class="region-badge" title="${i.name}">${K[i.code] ?? "🌍"}</span>`
    )}
    `;
  }
  _renderMethod(e, i = !1) {
    return a`
      <div class="method-row ${i ? "outranked" : ""}">
        ...
        <div class="method-provider">
          ${i ? a`<span class="outranked-text">outranked by ${e.outrankedBy}</span>` : a`<span class="via-text">via ${e.providerDisplayName}</span>`}
        </div>
      </div>
    `;
  }
  _renderSection(e, i, r = !1) {
    return i.length === 0 ? s : a`
      <div class="preview-section">
        <h4 class="section-title">${e}</h4>
        <div class="methods-list">
          ${i.map((t) => this._renderMethod(t, r))}
        </div>
      </div>
    `;
  }
      ...
      <div class="preview-box">
        ...
        ${this._isCollapsed ? s : a`
              <div class="preview-content">
                ...
                ${this._preview?.hiddenMethods.length ? a`
...
_([
  O({ type: Boolean, attribute: "auto-load" })
...
var J = Object.defineProperty, Q = Object.getOwnPropertyDescriptor, I = (e) => {
  ...
  for (var n = t > 1 ? void 0 : t ? Q(i, r) : i, h = e.length - 1, g; h >= 0; h--)
    (g = e[h]) && (n = (t ? g(i, r, n) : g(n)) || n);
  return t && n && J(i, r, n), n;
}, R = (e, i, r) => i.has(e) || I("Cannot " + r), o = (e, i, r) => (R(e, i, "read from private field"), i.get(e)), y = (e, i, r) => i.has(e) ? I("Cannot add the same private member more than once") : i instanceof WeakSet ? i.add(e) : i.set(e, r), w = (e, i, r, t) => (R(e, i, "write to private field"), i.set(e, r), r), d, u, l, P;
let c = class extends L(k) {
  constructor() {
    super(), this._availableProviders = [], this._configuredProviders = [], this._isLoading = !0, this._errorMessage = null, y(this, d), y(this, u), y(this, l, !1), y(this, P, new H(this, {
  ...
  async _loadProviders() {
    ...
    try {
      const [e, i] = await Promise.all([
        f.getAvailablePaymentProviders(),
        f.getPaymentProviders()
      ]);
      if (!o(this, l)) return;
      if (e.error) {
        this._errorMessage = e.error.message, this._isLoading = !1;
  ...
  async _handleProviderReorder(e) {
    const { error: i } = await f.reorderPaymentProviders(e);
  ...
  async _openMethodsModal(e) {
    if (!o(this, d)) return;
    (await o(this, d).open(this, j, {
      data: { setting: e }
    }).onSubmit().catch(() => {
    }))?.isChanged && await this._previewElement?.loadPreview();
  }
  async _toggleProvider(e) {
    const { error: i } = await f.togglePaymentProvider(e.id, !e.isEnabled);
    if (o(this, l)) {
      if (i) {
  ...
  async _deleteProvider(e) {
    ...
    if (!o(this, l)) return;
    const { error: r } = await f.deletePaymentProvider(e.id);
    if (o(this, l)) {
      if (r) {
        o(this, u)?.peek("danger", {
  ...
  _openSetupInstructions(e) {
    !o(this, d) || !e.setupInstructions || o(this, d).open(this, B, {
      data: {
  ...
  _openTestModal(e) {
    o(this, d) && o(this, d).open(this, q, {
      data: { setting: e }
    });
  }
  _renderProviderIcon(e, i, r) {
    const t = i ?? W(e);
    return t ? a`<span class="provider-icon-svg">${C(t)}</span>` : a`<uui-icon name="${r ?? "icon-credit-card"}"></uui-icon>`;
  }
  _renderConfiguredProvider(e) {
    ...
    return a`
      <div class="provider-card configured" data-provider-id=${e.id}>
        ...
        ${i?.description ? a`<p class="provider-description">${i.description}</p>` : s}
        <div class="provider-footer">
          <div class="provider-features">
            ${e.isTestMode ? a`<span class="feature-badge test-mode">Test Mode</span>` : a`<span class="feature-badge live-mode">Live</span>`}
            ${i?.supportsRefunds ? a`<span class="feature-badge">Refunds</span>` : s}
            ${i?.supportsPartialRefunds ? a`<span class="feature-badge">Partial Refunds</span>` : s}
            ${i?.usesRedirectCheckout ? a`<span class="feature-badge">Redirect Checkout</span>` : s}
            ${i?.supportsAuthAndCapture ? a`<span class="feature-badge">Auth & Capture</span>` : s}
          </div>
          ${i?.setupInstructions ? a`
                <uui-button
                  look="secondary"
                  compact
  ...
  _renderAvailableProvider(e) {
    return a`
      <div class="provider-card available">
        ...
        <div class="provider-footer">
          ${e.description ? a`<p class="provider-description">${e.description}</p>` : s}
  ...
  render() {
    ...
    return a`
      ...
      <div class="content">
        ${this._isLoading ? a`<div class="loading">
              <uui-loader></uui-loader>
              <span>Loading payment providers...</span>
            </div>` : this._errorMessage ? a`<uui-box>
                <div class="error">
                  <uui-icon name="icon-alert"></uui-icon>
                  <span>${this._errorMessage}</span>
        ...
        ${e ? a`<merchello-checkout-payment-preview></merchello-checkout-payment-preview>` : s}
        ...
        <uui-box headline="Configured Payment Providers" ?hidden=${!e}>
          ...
          <div class="providers-list configured">
            ${e ? this._configuredProviders.length === 0 ? a`<p class="no-items">No payment providers configured yet.</p>` : this._configuredProviders.map((r) => this._renderConfiguredProvider(r)) : s}
          </div>
        </uui-box>

        ${e ? a`<uui-box headline="Available Payment Providers">
              <p class="section-description">
                These payment providers are available but not yet configured.
                Click "Install" to configure and add a provider.
              </p>
              ${i.length === 0 ? a`<p class="no-items">All available providers have been configured.</p>` : a`
...
b([
  T("merchello-checkout-payment-preview")
...
//# sourceMappingURL=payment-providers-list.element-koI4DK8H.js.map
