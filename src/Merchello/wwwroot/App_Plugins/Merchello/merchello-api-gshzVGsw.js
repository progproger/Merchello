...
async function t(e) {
  try {
    const r = await h(), i = p.baseUrl || "", s = await fetch(`${i}${u}/${e}`, {
      method: "GET",
    ...
    let c;
    return a.includes("application/json") ? c = await s.json() : c = await s.text(), { data: c };
  } catch (r) {
...
async function n(e, r) {
  try {
    const i = await h(), s = p.baseUrl || "", a = await fetch(`${s}${u}/${e}`, {
      method: "POST",
      credentials: p.credentials,
      headers: i,
...
async function d(e) {
  try {
    ...
    if (!s.ok) {
      ...
      return { error: new Error(a || `HTTP ${s.status}: ${s.statusText}`) };
    }
    return {};
  } catch (r) {
    return { error: r instanceof Error ? r : new Error(String(r)) };
...
const v = {
  ...
  /** Update a variant */
  updateVariant: (e, r, i) => o(`products/${e}/variants/${r}`, i),
  ...
  updateServiceRegion: (e, r, i) => o(`warehouses/${e}/service-regions/${r}`, i),
...
//# sourceMappingURL=merchello-api-gshzVGsw.js.map
