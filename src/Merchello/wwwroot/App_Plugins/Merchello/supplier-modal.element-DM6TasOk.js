import { nothing as p, html as r, css as C, state as n, customElement as g } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalBaseElement as S } from "@umbraco-cms/backoffice/modal";
import { M as v } from "./merchello-api-Dp_zU_yi.js";
var y = Object.defineProperty, b = Object.getOwnPropertyDescriptor, a = (e, t, s, i) => {
  for (var l = i > 1 ? void 0 : i ? b(t, s) : t, u = e.length - 1, d; u >= 0; u--)
    (d = e[u]) && (l = (i ? d(t, s, l) : d(l)) || l);
  return i && l && y(t, s, l), l;
};
const P = "supplier-direct", _ = [
...
let o = class extends S {
  ...
  _validate() {
    ...
    if (this._parseEmailList(this._emailCcAddresses).some((i) => !this._validateEmail(i)) && (e.emailCcAddresses = "One or more CC email addresses are invalid"), t && this._deliveryMethod === "Email" && !this._validateEmail(this._emailRecipient.trim()) && (e.emailRecipient = "Supplier recipient email format is invalid"), t && this._deliveryMethod === "Email" && !this._emailRecipient.trim() && !this._contactEmail.trim() && (e.emailRecipient = "Provide supplier recipient email or supplier contact email"), t && this._deliveryMethod !== "Email" && this._ftpPort.trim() && Number.isNaN(Number(this._ftpPort.trim())) && (e.ftpPort = "FTP/SFTP port must be a valid number"), t && this._deliveryMethod !== "Email" && !this._ftpHost.trim() && (e.ftpHost = "FTP/SFTP host is required"), t && this._deliveryMethod !== "Email" && !this._ftpUsername.trim() && (e.ftpUsername = "FTP/SFTP username is required"), t && this._deliveryMethod !== "Email" && !this._isEditMode && !this._ftpPassword.trim() && (e.ftpPassword = "FTP/SFTP password is required when creating a supplier"), t && this._deliveryMethod !== "Email" && this._useCustomCsvSettings) {
      ...
      const u = this._normalizeCsvColumns(this._csvColumns), d = this._normalizeCsvStaticColumns(this._csvStaticColumns);
      u.length === 0 && d.length === 0 && (e.csvColumns = "Add at least one CSV column or static column, or disable custom CSV format.");
      const f = /* @__PURE__ */ new Set();
      for (const c of u) {
        const m = c.field.toLowerCase();
        if (f.has(m)) {
  ...
  async _handleSave() {
    ...
    if (this._isEditMode) {
      ...
      const { data: f, error: h } = await v.updateSupplier(d, i);
  ...
  _renderFormLayoutItem(e) {
    return r`
      <uui-form-layout-item>
        ...
        ${e.input}
        ${e.hint ? r`<span class="hint">${e.hint}</span>` : p}
        ${e.error ? r`<span class="error" role="alert">${e.error}</span>` : p}
      </uui-form-layout-item>
    `;
  }
  _renderCsvSettingsSection() {
    return r`
      ...
      (e, t) => r`
              ...
              ${this._errors.csvColumns ? r`<span class="error">${this._errors.csvColumns}</span>` : p}
      ...
      (e, t) => r`
        ...
        s.target.value
              ...
              ${this._errors.csvStaticColumns ? r`<span class="error">${this._errors.csvStaticColumns}</span>` : p}
            ` : r`
    ...
    return r`
      ...
      hint: "Recommended when strict host verification is enabled."
        ...
        <div class="connection-test">
          ...
          ${this._ftpConnectionTestError ? r`<span class="error" role="alert">${this._ftpConnectionTestError}</span>` : p}
      ...
      <umb-body-layout headline=${e}>
        <uui-form>
          <form id="SupplierForm" @submit=${this._handleFormSubmit}>
            <div id="main">
              ${this._errors.general ? r`<div class="error-banner" role="alert">${this._errors.general}</div>` : p}
...
//# sourceMappingURL=supplier-modal.element-DM6TasOk.js.map
