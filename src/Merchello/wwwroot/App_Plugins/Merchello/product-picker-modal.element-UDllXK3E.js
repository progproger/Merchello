import { LitElement as C, html as s, nothing as c, css as P, property as b, state as p, customElement as A } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalBaseElement as T } from "@umbraco-cms/backoffice/modal";
import { M as w } from "./merchello-api-B76CV0sD.js";
import { b as L } from "./formatting-MfE1tvkN.js";
...
var j = Object.defineProperty, V = Object.getOwnPropertyDescriptor, $ = (e, t, i, a) => {
  for (var o = a > 1 ? void 0 : a ? V(t, i) : t, r = e.length - 1, n; r >= 0; r--)
    (n = e[r]) && (o = (a ? n(t, i, o) : n(o)) || o);
  return a && o && j(t, i, o), o;
};
let k = class extends z(C) {
  constructor() {
  ...
  _renderSku() {
    return this.variant.sku ? s`<span class="variant-sku">${this.variant.sku}</span>` : c;
  }
  _renderPrice() {
   ...
   */
  _renderStockStatus() {
    if (!this.variant.stockStatusLabel) return c;
  ...
  _renderRegionStatus() {
    return this.variant.canShipToRegion ? c : s`<span class="status blocked">${this.variant.regionMessage ?? "Cannot ship"}</span>`;
  }
...
$([
  b({ type: String })
], k.prototype, "currencySymbol", 2);
$([
  b({ type: Boolean })
], k.prototype, "showImage", 2);
$([
  p()
], k.prototype, "_imageFailed", 2);
k = $([
  A("merchello-product-picker-variant-row")
], k);
var U = Object.defineProperty, B = Object.getOwnPropertyDescriptor, y = (e, t, i, a) => {
  for (var o = a > 1 ? void 0 : a ? B(t, i) : t, r = e.length - 1, n; r >= 0; r--)
    (n = e[r]) && (o = (a ? n(t, i, o) : n(o)) || o);
  return a && o && U(t, i, o), o;
};
let _ = class extends z(C) {
  constructor() {
  ...
  _renderPriceRange(e) {
    return N(e.minPrice, e.maxPrice, this.currencySymbol);
  }
  /**
   ...
   */
  _renderStockBadge(e) {
    if (!e.stockStatusLabel) return c;
        ...
        >
          ...
          <div class="product-info">
            ...
            <div class="product-meta">
              ...
              ${t ? c : s`<span class="variant-count">${e.variantCount} variants</span>`}
...
y([
  b({ type: Boolean })
], _.prototype, "selectRoots", 2);
y([
  b({ type: Array })
], _.prototype, "selectedRootIds", 2);
y([
  p()
], _.prototype, "_failedImages", 2);
_ = y([
  A("merchello-product-picker-list")
], _);
var F = Object.defineProperty, W = Object.getOwnPropertyDescriptor, h = (e, t, i, a) => {
  for (var o = a > 1 ? void 0 : a ? W(t, i) : t, r = e.length - 1, n; r >= 0; r--)
    (n = e[r]) && (o = (a ? n(t, i, o) : n(o)) || o);
  return a && o && F(t, i, o), o;
};
let u = class extends T {
  ...
  async _loadVariantsForRoot(e) {
    ...
    if (t === -1) return;
    const { data: i, error: a } = await w.getProductDetail(e);
    if (a || !i)
      return;
    this._productDetailCache.set(e, i);
    const o = await Promise.all(
      i.variants.map(async (l) => this._mapToPickerVariant(l, i))
    ), r = i.variants.reduce((l, f) => l + f.totalStock, 0), n = i.aggregateStockStatus, m = i.aggregateStockStatusLabel ?? "", g = i.aggregateStockStatusCssClass ?? "";
    this._productRoots = this._productRoots.map(
      (l, f) => f === t ? {
        ...
        stockStatus: n,
        stockStatusLabel: m,
        stockStatusCssClass: g
      } : l
    );
  }
  async _mapToPickerVariant(e, t) {
    const i = e.totalStock, a = e.warehouseStock.some((d) => d.trackStock);
    let o = !0, r = null, n = !0, m = null, g = null, l = e.stockStatus, f = e.stockStatusLabel, x = e.stockStatusCssClass;
    if (!this._isPropertyEditorMode) {
      if (this._config?.shippingAddress && !t.isDigitalProduct) {
        const d = await this._getFulfillmentOptions(e.id);
        o = d.canAddToOrder, r = d.blockedReason, n = d.canAddToOrder, m = d.warehouseId, g = d.warehouseName, d.aggregateStockStatusLabel && d.aggregateStockStatusCssClass && (l = d.aggregateStockStatus, f = d.aggregateStockStatusLabel, x = d.aggregateStockStatusCssClass);
      } else if (!t.isDigitalProduct && e.warehouseStock.length > 0) {
        const d = await this._getDefaultFulfillingWarehouse(e.id);
        o = d.canAddToOrder, r = d.blockedReason, m = d.warehouseId, g = d.warehouseName, d.aggregateStockStatusLabel && d.aggregateStockStatusCssClass && (l = d.aggregateStockStatus, f = d.aggregateStockStatusLabel, x = d.aggregateStockStatusCssClass);
      }
    }
    let S = l, R = f, I = x;
    if (m) {
      const d = e.warehouseStock.find((O) => O.warehouseId === m);
      d && (S = d.stockStatus, R = d.stockStatusLabel, I = d.stockStatusCssClass);
    }
    return {
      id: e.id,
      productRootId: t.id,
  ...
  async _getFulfillmentOptions(e) {
    ...
    try {
      const { data: i, error: a } = await w.getProductFulfillmentOptions(
        e,
        t.countryCode,
        t.regionCode
      );
  ...
  async _getDefaultFulfillingWarehouse(e) {
    try {
      const { data: t, error: i } = await w.getDefaultFulfillingWarehouse(e);
      return i ? {
        canAddToOrder: !1,
        warehouseId: null,
        warehouseName: null,
  ...
  _handleAddonSelect(e, t) {
    ...
    }, a = this._selectedAddons.get(e.id) ?? [], o = a.some((r) => r.valueId === t.id);
    if (e.isMultiSelect) {
      const r = o ? a.filter((n) => n.valueId !== t.id) : [...a, i];
      r.length === 0 ? this._selectedAddons.delete(e.id) : this._selectedAddons.set(e.id, r);
    } else
      o ? this._selectedAddons.delete(e.id) : this._selectedAddons.set(e.id, [i]);
    this._selectedAddons = new Map(this._selectedAddons), this._fetchAddonPricePreviewDebounced();
  }
  _handleAddonClear(e) {
  ...
  _getMissingRequiredAddonOptions() {
    const e = this._pendingAddonSelection;
    return e ? e.addonOptions.filter((t) => t.isRequired ? (this._selectedAddons.get(t.id) ?? []).length === 0 : !1) : [];
  }
  /**
  ...
  async _fetchAddonPricePreview() {
    ...
    const t = this._getSelectedAddons();
    if (t.length === 0) {
    ...
    const i = {
      selectedAddons: t.map((r) => ({
        optionId: r.optionId,
        valueId: r.valueId
      }))
    }, { data: a, error: o } = await w.previewAddonPrice(e.variant.id, i);
    this._pendingAddonSelection && (o ? (this._addonPricePreview = null, this._addonPreviewError = "Unable to calculate price. Please try again.") : a && (this._addonPricePreview = {
  ...
  _handleSkipAddons() {
    this._pendingAddonSelection && (this._getMissingRequiredAddonOptions().length > 0 || (this._transitionToShippingSelection(this._pendingAddonSelection.variant, []), this._pendingAddonSelection = null, this._selectedAddons = /* @__PURE__ */ new Map()));
  }
  _handleConfirmWithAddons() {
    if (!this._pendingAddonSelection || this._getMissingRequiredAddonOptions().length > 0) return;
    const e = this._getSelectedAddons();
  ...
  async _transitionToShippingSelection(e, t) {
    ...
    const { data: n, error: m } = await w.getShippingOptionsForWarehouse(
      i,
      r.countryCode,
      r.regionCode
    );
    ...
    const g = n.availableOptions.map((l) => ({
      id: l.id,
      name: l.name,
    ...
    this._pendingShippingSelection = {
      ...
      isLoadingOptions: !1,
      shippingOptions: g
    }, g.length === 1 && (this._selectedShippingOptionId = g[0].id);
  }
  ...
  _handleConfirmWithShipping() {
    ...
    if (!e) return;
    const { variant: t, addons: i, warehouseId: a, warehouseName: o } = this._pendingShippingSelection, r = {
      productId: t.id,
    ...
    this._selections.set(t.id, r), this._selections = new Map(this._selections), this._viewState = "product-selection", this._pendingShippingSelection = null, this._selectedShippingOptionId = null;
  }
  _handleAdd() {
  ...
  _renderAddonSelectionView() {
    const e = this._pendingAddonSelection;
    if (!e) return c;
    const t = e.variant, i = t.optionValuesDisplay ? `${t.rootName} - ${t.optionValuesDisplay}` : t.rootName, a = this._addonPricePreview, o = this._addonPreviewError !== null, r = a?.basePrice ?? t.price, n = a?.addonsTotal ?? 0, m = a?.totalPrice ?? t.price, g = this._getMissingRequiredAddonOptions(), l = g.length > 0, f = e.addonOptions.some((S) => S.isRequired), x = !o && !this._isLoadingAddonPreview && !l;
    return s`
      <umb-body-layout headline="${f ? "Select Add-ons" : "Select Add-ons (Optional)"}">
        <div id="main">
          ${o ? s`
          ...
          ${l ? s`
                <div class="addon-error">
                  <uui-icon name="icon-alert"></uui-icon>
                  <span>
                    Select required add-on${g.length === 1 ? "" : "s"}:
                    ${g.map((S) => S.name).join(", ")}
                  </span>
                </div>
              ` : c}
          ...
          <div class="addon-product-summary">
            <div class="product-info">
              <strong>${i}</strong>
              ${t.sku ? s`<span class="sku">${t.sku}</span>` : c}
            </div>
            <div class="product-pricing ${this._isLoadingAddonPreview ? "loading" : ""} ${o ? "error" : ""}">
              ...
              ${!o && n !== 0 ? s`
                  ...
                  ` : c}
              ${this._isLoadingAddonPreview ? s`<uui-loader-circle></uui-loader-circle>` : c}
            </div>
          </div>

          <div class="addon-options">
            ${e.addonOptions.map((S) => this._renderAddonOption(S))}
          </div>
        </div>
  ...
  _renderAddonOption(e) {
    const t = this._selectedAddons.get(e.id) ?? [], i = t.length > 0, a = e.isMultiSelect ? "multi-select" : "single-select", o = e.isRequired ? "required" : "optional";
    return s`
      <div class="addon-option">
        <div class="addon-option-header">
          <span class="addon-option-name">${e.name}</span>
          <span class="addon-optional">(${o}, ${a})</span>
          ${i ? s`
        ...
        <div class="addon-values">
          ${e.values.map(
      (r) => this._renderAddonValue(e, r, t.some((n) => n.valueId === r.id))
    )}
        </div>
      </div>
    `;
  ...
  _renderAddonValue(e, t, i) {
    return s`
      <button
        ...
        @click=${() => this._handleAddonSelect(e, t)}
      >
      ...
      >
        ...
        <div class="shipping-option-cost">
          ${e.estimatedCost !== null ? s`
                ...
                ${e.isEstimate ? s`<span class="estimate-label">est.</span>` : c}
...
u = h([
  A("merchello-product-picker-modal")
], u);
const Q = u;
export {
...
//# sourceMappingURL=product-picker-modal.element-UDllXK3E.js.map
