...
async function t(e) {
  try {
    ...
    let c;
    return n.includes("application/json") ? c = await a.json() : c = await a.text(), { data: c };
  } catch (r) {
...
async function o(e, r) {
  try {
    ...
    if (!n.ok) {
      ...
      return { error: new Error(g || `HTTP ${n.status}: ${n.statusText}`) };
    }
...
const v = {
  ...
  getProductShippingOptions: (e) => t(`products/${e}/shipping-options`),
  /** Update shipping exclusions for all variants (bulk mode) */
  updateProductShippingExclusions: (e, r) => s(`products/${e}/shipping-exclusions`, {
    excludedShippingOptionIds: r
  }),
  /** Update shipping exclusions for a specific variant */
  updateVariantShippingExclusions: (e, r, i) => s(`products/${e}/variants/${r}/shipping-exclusions`, {
  ...
  getCustomers: (e) => {
    const r = h(e);
    return t(`customers${r ? `?${r}` : ""}`);
  },
  /** Get a single customer by ID */
  getCustomer: (e) => t(`customers/${e}`),
  /** Update an existing customer */
  updateCustomer: (e, r) => s(`customers/${e}`, r),
  // ============================================
...
//# sourceMappingURL=merchello-api-DgfpLvp2.js.map
