import { html as t, nothing as o, css as _, state as n, customElement as m } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalBaseElement as g } from "@umbraco-cms/backoffice/modal";
import { c as d } from "./formatting-BB_-NCdW.js";
import { M as c } from "./merchello-api-B1skiL_A.js";
var v = Object.defineProperty, f = Object.getOwnPropertyDescriptor, r = (i, e, a, l) => {
  for (var p = l > 1 ? void 0 : l ? f(e, a) : e, h = i.length - 1, u; h >= 0; h--)
    (u = i[h]) && (p = (l ? u(e, a, p) : u(p)) || p);
  return l && p && v(e, a, p), p;
};
let s = class extends g {
  constructor() {
    super(...arguments), this._name = "", this._sku = "", this._price = 0, this._quantity = 1, this._selectedTaxGroupId = null, this._isPhysicalProduct = !0, this._errors = {}, this._taxPreview = null, this._isLoadingTaxPreview = !1, this._taxPreviewDebounceTimer = null, this._step = "details", this._warehouses = [], this._isLoadingWarehouses = !1, this._selectedWarehouseId = null, this._shippingOptions = [], this._isLoadingShippingOptions = !1, this._selectedShippingOptionId = null, this._shippingError = null;
  }
  ...
  _validateDetails() {
    const i = {};
    return this._name.trim() || (i.name = "Item name is required"), this._sku.trim() || (i.sku = "SKU is required"), this._price <= 0 && (i.price = "Price must be greater than 0"), this._quantity < 1 && (i.quantity = "Quantity must be at least 1"), this._errors = i, Object.keys(i).length === 0;
  }
  _validateShipping() {
  ...
  async _loadWarehouses() {
    ...
    try {
      ...
      e ? (console.error("Failed to load warehouses:", e), this._warehouses = []) : this._warehouses = i ?? [];
    } catch (i) {
      console.error("Failed to load warehouses:", i), this._warehouses = [];
    } finally {
  ...
  async _loadShippingOptions() {
    ...
    try {
      const { data: e, error: a } = await c.getShippingOptionsForWarehouse(
        this._selectedWarehouseId,
        i.countryCode,
        i.stateCode
      );
      if (a || !e) {
        console.error("Failed to load shipping options:", a), this._shippingError = "Failed to load shipping options";
        return;
      }
      ...
      console.error("Failed to load shipping options:", e), this._shippingError = "Failed to load shipping options";
    } finally {
  ...
  _submitItem() {
    const i = this._warehouses.find((a) => a.id === this._selectedWarehouseId), e = this._shippingOptions.find((a) => a.id === this._selectedShippingOptionId);
    this.value = {
      item: {
  ...
  _refreshTaxPreview() {
    this._taxPreviewDebounceTimer && clearTimeout(this._taxPreviewDebounceTimer), this._taxPreviewDebounceTimer = setTimeout(async () => {
      ...
      try {
        ...
        e ? (console.error("Failed to preview tax:", e), this._taxPreview = null) : i && (this._taxPreview = {
          subtotal: i.subtotal,
          taxRate: i.taxRate,
        ...
        console.error("Unexpected error previewing tax:", i), this._taxPreview = null;
      } finally {
  ...
  _renderDetailsStep() {
    const i = this.data?.currencySymbol ?? "£", e = this._taxPreview !== null, a = this._taxPreview?.taxRate, l = this._taxPreview?.subtotal, p = this._taxPreview?.taxAmount, h = this._taxPreview?.total;
    return t`
      <div class="form-row-group">
        <div class="form-row">
          ...
          ></uui-input>
          ${this._errors.name ? t`<span class="error">${this._errors.name}</span>` : o}
        </div>
        ...
        <div class="form-row">
          ...
          ></uui-input>
          ${this._errors.sku ? t`<span class="error">${this._errors.sku}</span>` : o}
        </div>
      </div>
      ...
      <div class="form-row-group">
        <div class="form-row">
          ...
          ${this._errors.price ? t`<span class="error">${this._errors.price}</span>` : o}
        </div>
        ...
        <div class="form-row">
          ...
          <uui-input
            ...
            min="1"
          ></uui-input>
          ${this._errors.quantity ? t`<span class="error">${this._errors.quantity}</span>` : o}
        </div>
      </div>
      ...
      <div class="form-row">
        ...
        ${this._selectedTaxGroupId && this._price > 0 ? t`
          <span class="tax-info">
            ${e && p !== void 0 && a !== void 0 ? `Tax: ${i}${d(p, 2)} at ${a}%` : t`<span class="calculating">Calculating tax...</span>`}
          </span>
        ` : o}
      </div>
        ...
        ${this._selectedWarehouseId ? t`
          <div class="form-row">
            ...
            ` : this._shippingOptions.length > 0 ? t`
              ...
              ></uui-select>
              ${this._selectedShippingOptionId ? this._renderSelectedShippingDetails() : o}
            ` : o}
          </div>
        ` : o}
  ...
  _renderSelectedShippingDetails() {
    const i = this._shippingOptions.find((a) => a.id === this._selectedShippingOptionId);
    if (!i) return o;
  ...
  _renderActions() {
    const i = this._step === "details", e = i && this._isPhysicalProduct, a = !i || !this._isPhysicalProduct;
    return t`
      <div slot="actions">
        ${!i ? t`
...
//# sourceMappingURL=add-custom-item-modal.element-BeMTkN8F.js.map
