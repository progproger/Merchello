import { LitElement as A, html as o, nothing as c, css as x, property as b, state as p, customElement as P } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalBaseElement as z } from "@umbraco-cms/backoffice/modal";
import { M as S } from "./merchello-api-BAKL0aIE.js";
import { c as O } from "./formatting-CG1-kUla.js";
...
let d = class extends z {
  ...
  async _loadVariantsForRoot(e) {
    const i = this._productRoots.findIndex((u) => u.id === e);
    if (i === -1) return;
    ...
    const a = await Promise.all(
      t.variants.map(async (u) => this._mapToPickerVariant(u, t))
    ), r = a.reduce((u, g) => u + g.availableStock, 0), n = t.aggregateStockStatus;
    this._productRoots = this._productRoots.map(
      (u, g) => g === i ? { ...u, variants: a, variantsLoaded: !0, totalStock: r, stockStatus: n } : u
    );
  }
  async _mapToPickerVariant(e, i) {
    ...
    let a = !0, r = null, n = !0, u = null, g = null, m = "InStock";
    if (!this._isPropertyEditorMode) {
      if (this._config?.shippingAddress && !i.isDigitalProduct) {
        ...
        a = l.canAddToOrder, r = l.blockedReason, n = l.canAddToOrder, u = l.warehouseId, g = l.warehouseName, m = l.aggregateStockStatus;
      } else if (!i.isDigitalProduct && e.warehouseStock.length > 0) {
        const l = await this._getDefaultFulfillingWarehouse(e.id);
        a = l.canAddToOrder, r = l.blockedReason, u = l.warehouseId, g = l.warehouseName, m = l.aggregateStockStatus;
      }
    }
    let $ = m;
    if (u) {
      const l = e.warehouseStock.find((w) => w.warehouseId === u);
      l && ($ = l.stockStatus);
    }
    return {
      id: e.id,
  ...
  async _transitionToShippingSelection(e, i) {
    ...
    const { data: n, error: u } = await S.getShippingOptionsForWarehouse(
      t,
      r.countryCode,
      r.stateCode
    );
  ...
  _renderAddonSelectionView() {
    const e = this._pendingAddonSelection;
    if (!e) return c;
    const i = e.variant, t = i.optionValuesDisplay ? `${i.rootName} - ${i.optionValuesDisplay}` : i.rootName, s = this._addonPricePreview, a = this._addonPreviewError !== null, r = s?.basePrice ?? i.price, n = s?.addonsTotal ?? 0, u = s?.totalPrice ?? i.price, g = !a && !this._isLoadingAddonPreview;
    return o`
          ...
          <div class="addon-product-summary">
            ...
            <div class="product-pricing ${this._isLoadingAddonPreview ? "loading" : ""} ${a ? "error" : ""}">
              ...
              ${!a && n !== 0 ? o`
                    ...
                    <span class="total-price">= ${v(u, this._currencySymbol)}</span>
                  ` : c}
...
//# sourceMappingURL=product-picker-modal.element-CDGtLfyF.js.map
