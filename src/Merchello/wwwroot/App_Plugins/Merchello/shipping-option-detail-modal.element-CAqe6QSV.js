import { html as r, nothing as n, css as D, state as o, customElement as x } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalToken as m, UmbModalBaseElement as w, UMB_MODAL_MANAGER_CONTEXT as k } from "@umbraco-cms/backoffice/modal";
import { UMB_NOTIFICATION_CONTEXT as S } from "@umbraco-cms/backoffice/notification";
import { M as h } from "./merchello-api-CCwReUh_.js";
const $ = new m("Merchello.ShippingCost.Modal", {
  modal: {
    type: "dialog",
    size: "medium"
  }
}), T = new m("Merchello.ShippingWeightTier.Modal", {
  modal: {
    type: "dialog",
    size: "medium"
  }
});
var C = Object.defineProperty, O = Object.getOwnPropertyDescriptor, _ = (i) => {
  throw TypeError(i);
}, s = (i, e, t, u) => {
  for (var l = u > 1 ? void 0 : u ? O(e, t) : e, v = i.length - 1, g; v >= 0; v--)
    (g = i[v]) && (l = (u ? g(e, t, l) : g(l)) || l);
  return u && l && C(e, t, l), l;
}, b = (i, e, t) => e.has(i) || _("Cannot " + t), d = (i, e, t) => (b(i, e, "read from private field"), e.get(i)), y = (i, e, t) => e.has(i) ? _("Cannot add the same private member more than once") : e instanceof WeakSet ? e.add(i) : e.set(i, t), f = (i, e, t, u) => (b(i, e, "write to private field"), e.set(i, t), t), p, c;
let a = class extends w {
  constructor() {
    super(), this._isLoading = !1, this._isSaving = !1, this._errorMessage = null, this._detail = null, this._warehouses = [], this._availableProviders = [], this._methodConfig = null, this._isLoadingProviders = !1, this._name = "", this._warehouseId = "", this._fixedCost = null, this._daysFrom = 3, this._daysTo = 5, this._isNextDay = !1, this._nextDayCutOffTime = "", this._allowsDeliveryDateSelection = !1, this._minDeliveryDays = null, this._maxDeliveryDays = null, this._allowedDaysOfWeek = "", this._isDeliveryDateGuaranteed = !1, this._providerKey = "flat-rate", this._serviceType = null, this._providerSettings = {}, this._isEnabled = !0, y(this, p), y(this, c), this._daysOfWeek = [
  ...
  async _loadDetailById(i) {
    ...
    try {
      ...
      if (e && (this._detail = e, this._name = e.name ?? "", this._hasFixedWarehouse || (this._warehouseId = e.warehouseId), this._fixedCost = e.fixedCost ?? null, this._daysFrom = e.daysFrom, this._daysTo = e.daysTo, this._isNextDay = e.isNextDay, this._nextDayCutOffTime = e.nextDayCutOffTime ?? "", this._allowsDeliveryDateSelection = e.allowsDeliveryDateSelection, this._minDeliveryDays = e.minDeliveryDays ?? null, this._maxDeliveryDays = e.maxDeliveryDays ?? null, this._allowedDaysOfWeek = e.allowedDaysOfWeek ?? "", this._isDeliveryDateGuaranteed = e.isDeliveryDateGuaranteed, this._providerKey = e.providerKey ?? "flat-rate", this._serviceType = e.serviceType ?? null, this._providerSettings = e.providerSettings ?? {}, this._isEnabled = e.isEnabled ?? !0, this._providerKey !== "flat-rate")) {
        ...
        const { data: u } = await h.getShippingProviderMethodConfig(this._providerKey);
        u && (this._methodConfig = u);
      }
    } catch (e) {
  ...
  async _openCostModal(i) {
    if (!d(this, p) || !this._detail) return;
    (await d(this, p).open(this, $, {
      data: { cost: i, optionId: this._detail.id, warehouseId: this._detail.warehouseId }
    }).onSubmit().catch(() => {
    }))?.isSaved && await this._loadDetail();
  }
  async _deleteCost(i) {
    if (!confirm(`Delete cost for ${i.regionDisplay ?? i.countryCode}?`)) return;
    const { error: e } = await h.deleteShippingCost(i.id);
    if (e) {
      d(this, c)?.peek("danger", {
  ...
  async _openWeightTierModal(i) {
    if (!d(this, p) || !this._detail) return;
    (await d(this, p).open(this, T, {
      data: { tier: i, optionId: this._detail.id, warehouseId: this._detail.warehouseId }
    }).onSubmit().catch(() => {
    }))?.isSaved && await this._loadDetail();
  }
  async _deleteWeightTier(i) {
    if (!confirm(`Delete weight tier ${i.weightRangeDisplay}?`)) return;
    const { error: e } = await h.deleteShippingWeightTier(i.id);
    if (e) {
      d(this, c)?.peek("danger", {
        data: { headline: "Error", message: e.message }
      });
  ...
  _toggleDay(i) {
    ...
    if (e.includes(i))
      ...
      const t = this._daysOfWeek.map((l) => l.value), u = [...e, i].sort(
        (l, v) => t.indexOf(l) - t.indexOf(v)
      );
      this._allowedDaysOfWeek = u.join(",");
    }
  }
    ...
    return this._detail ? r`
      ...
      (i) => r`
                      <tr>
                        ...
                        <td class="cost-cell">$${i.cost.toFixed(2)}</td>
    ...
    return this._detail ? r`
      ...
      (i) => r`
                      <tr>
                        ...
                        <td class="cost-cell">+$${i.surcharge.toFixed(2)}</td>
                        <td class="actions-col">
  ...
  render() {
    ...
    return this._isLoading ? r`
      ...
      if (this._serviceType = e.target.value || null, !this._name && this._serviceType) {
        const u = this._methodConfig?.fields.find((l) => l.key === "serviceType")?.options?.find((l) => l.value === this._serviceType);
        u && (this._name = u.label);
      }
    }}
                      ></uui-select>
      ...
      this._maxDeliveryDays = t ? parseInt(t) : null;
          ...
          ${this._detail && !this._usesLiveRates ? this._renderCostsTable() : n}
          ${this._detail && !this._usesLiveRates ? this._renderWeightTiersTable() : n}
...
//# sourceMappingURL=shipping-option-detail-modal.element-CAqe6QSV.js.map
