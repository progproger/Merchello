...
export class MerchelloPropertyEditorUiCollectionPickerElement
  ...
  implements UmbPropertyEditorUiElement
{
  ...
  @state()
  private _selection: SelectedCollection[] = [];
  ...
  @state()
  private _allCollections: ProductCollectionDto[] = [];
  ...
  #sorter = new UmbSorterController<SelectedCollection>(this, {
  ...
  async #loadSelectionFromValue(val: string | undefined): Promise<void> {
    ...
    if (this._allCollections.length === 0) {
      ...
      const { data } = await MerchelloApi.getProductCollections();
    ...
    // Map IDs to full collection info, marking missing items as notFound
    const selection: SelectedCollection[] = [];
    for (const id of ids) {
      const collection = this._allCollections.find((c) => c.id === id);
      if (collection) {
        selection.push({
          id: collection.id,
        ...
        // Collection was deleted - keep the ID but mark as not found
        selection.push({
          id,
  ...
  async #openPicker(): Promise<void> {
    ...
    const newItems: SelectedCollection[] = result.selectedIds.map((id, index) => {
      const collection = this._allCollections.find((c) => c.id === id);
      return {
        id,
        name: result.selectedNames[index] ?? collection?.name ?? "Unknown",
        productCount: result.selectedCounts?.[index] ?? collection?.productCount ?? 0,
        notFound: false,
      };
    });
      ...
      return html`
        <div class="single-select-display not-found">
          <div class="collection-info">
            ...
            <span class="collection-name">${selected.name}</span>
            <span class="collection-id">ID: ${selected.id}</span>
          </div>
          ${!this.readonly
            ? html`
                <div class="actions">
    ...
    return html`
      <div class="single-select-display">
        <div class="collection-info">
          ...
          <span class="collection-name">${selected.name}</span>
          <span class="product-count">(${selected.productCount} products)</span>
        </div>
        ${!this.readonly
          ? html`
              <div class="actions">
    ...
    return html`
      <div class="collection-list">
        ${repeat(
          this._selection,
          (item) => item.id,
          (item) => this.#renderCollectionItem(item)
        )}
      </div>
      ${canAddMore
        ? html`
  ...
  #renderCollectionItem(item: SelectedCollection): unknown {
    if (item.notFound) {
      return html`
        <div class="collection-item not-found" data-id=${item.id}>
          <uui-icon name="icon-navigation" class="drag-handle"></uui-icon>
          <uui-icon name="icon-alert"></uui-icon>
          <span class="collection-name">${item.name}</span>
          <span class="collection-id">ID: ${item.id}</span>
          ${!this.readonly
            ? html`
                <uui-button
                  compact
    ...
    return html`
      <div class="collection-item" data-id=${item.id}>
        ...
        <uui-icon name="icon-folder"></uui-icon>
        <span class="collection-name">${item.name}</span>
        <span class="product-count">(${item.productCount} products)</span>
        ${!this.readonly
          ? html`
              <uui-button
                compact
...
export default MerchelloPropertyEditorUiCollectionPickerElement;

declare global {
  interface HTMLElementTagNameMap {
    "merchello-property-editor-ui-collection-picker": MerchelloPropertyEditorUiCollectionPickerElement;
  }
}
