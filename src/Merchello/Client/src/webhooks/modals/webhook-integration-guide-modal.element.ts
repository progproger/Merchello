...
@customElement("merchello-webhook-integration-guide-modal")
export class MerchelloWebhookIntegrationGuideModalElement extends UmbModalBaseElement<
  WebhookIntegrationGuideModalData,
  WebhookIntegrationGuideModalValue
> {
  ...
  private async _renderMarkdown(): Promise<void> {
    ...
    const parsed = await marked.parse(INTEGRATION_GUIDE_CONTENT);
