/**
 * Simple brand icons for payment methods and providers (symbol only, no text).
 * These are compact SVG icons suitable for UI lists and previews.
 */
...
/**
 * Gets the brand icon SVG for a payment method based on its alias.
 * @param methodAlias - The method alias (e.g., "cards", "paypal", "applepay")
 * @returns The SVG string or null if no matching icon
 */
export function getBrandIconSvg(methodAlias: string): string | null {
  ...
  if (alias.includes('paypal')) return BRAND_ICONS.paypal;
  if (alias.includes('apple')) return BRAND_ICONS.apple;
  if (alias.includes('google')) return BRAND_ICONS.google;
  if (alias.includes('venmo')) return BRAND_ICONS.venmo;
  if (alias.includes('link')) return BRAND_ICONS.link;
...
/**
 * Gets the brand icon SVG for a payment provider based on its alias.
 * @param providerAlias - The provider alias (e.g., "stripe", "braintree", "paypal")
 * @returns The SVG string or null if no matching icon
 */
